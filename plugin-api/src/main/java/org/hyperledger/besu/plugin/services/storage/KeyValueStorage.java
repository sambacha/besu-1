 ...
 *
 * <p>Behaviour expected with regard to key to value mapping is that of a map,
 * one key maps to one value, when a new value is added with an existing key,
 * that key now points at the new value.
 *
...
public interface KeyValueStorage extends Closeable {
   ...
   *
   * @throws StorageException problem encountered when attempting to clear
   *     storage.
   */
   ...
   * @return <code>true</code> when the given key is present in keyset,
   *     <code>false</code> otherwise.
   * @throws StorageException problem encountered when interacting with the key
   *     set.
   */
  ...
  /**
   * Delete the value corresponding to the given key if a write lock can be
   * instantly acquired on the underlying storage. Do nothing otherwise.
   *
   * @param key The key to delete.
   * @throws StorageException any problem encountered during the deletion
   *     attempt.
   * @return false if the lock on the underlying storage could not be instantly
   *     acquired, true otherwise
   */
  boolean tryDelete(byte[] key) throws StorageException;

  /**
   * Performs an evaluation against each key in the store, returning the set of
   * entries that pass.
   *
   * @param returnCondition predicate to evaluate each key against, unless the
   *     result is {@code null}, the key is added to the returned list of keys.
   * @return the set of keys that pass the condition.
   */
  Set<byte[]> getAllKeysThat(Predicate<byte[]> returnCondition);

  /**
   * Begins a fresh transaction, for sequencing operations for later atomic
   * execution.
   *
   * @return transaciton to sequence key-value operations.
   * @throws StorageException problem encountered when starting a new
   *     transaction.
   */
