...
/**
 * This class describes the behaviour of the Kubernetes NAT manager. Kubernetes
 * Nat manager add support for Kubernetes’s NAT implementation when Besu is
 * being run from a Kubernetes cluster
 */
public class KubernetesNatManager extends AbstractNatManager {
  ...
  public KubernetesNatManager() { super(NatMethod.KUBERNETES); }
  ...
  protected void doStart() throws NatInitializationException {
    ...
    try {
      ...
      final V1Service service =
          api.listServiceForAllNamespaces(null, null, null, null, null, null,
                                          null, null, null)
              .getItems()
              .stream()
              .filter(v1Service
                      -> v1Service.getMetadata().getName().contains(
                          DEFAULT_BESU_POD_NAME_FILTER))
              .findFirst()
              .orElseThrow(
                  () -> new NatInitializationException("Service not found"));
    ...
    try {
      ...
      service.getSpec().getPorts().forEach(v1ServicePort -> {
        try {
          ...
          forwardedPorts.add(new NatPortMapping(
              natServiceType,
              natServiceType.equals(NatServiceType.DISCOVERY)
                  ? NetworkProtocol.UDP
                  : NetworkProtocol.TCP,
              internalHost, internalAdvertisedHost, v1ServicePort.getPort(),
      ...
      throw new RuntimeException(
          "Failed update information using pod metadata : " + e.getMessage(),
          e);
    }
  }
