 ...
 *
 * <p>It keeps track of the interactions with each peer, including the
 * expectations of what we expect to receive next from each peer. In other
 * words, it implements the state machine for (discovery) peers.
 *
 * <p>When necessary, it updates the underlying {@link PeerTable}, particularly
 * with additions which may succeed or not depending on the contents of the
 * target bucket for the peer.
 *
 ...
 *
 * <ul>
 *   <li><em>KNOWN:</em> the peer is known but there is no ongoing interaction
 * with it. <li><em>BONDING:</em> an attempt to bond is being made (e.g. a PING
 * has been sent). <li><em>BONDED:</em> the bonding handshake has taken place
 * (e.g. an expected PONG has been received after having sent a PING or a PING
 * has been received and a PONG has been sent in response). This is the same as
 * having an "active" channel. <li><em>MESSAGE_EXPECTED (*)</em>: a message has
 * been sent and a response is expected. <li><em>DROPPED (*):</em> the peer is
 * no longer in our peer table.
 * </ul>
 *
 * <p>(*) It is worthy to note that the <code>MESSAGE_EXPECTED</code> and
 * <code>DROPPED</code> states are not modelled explicitly in {@link
 * PeerDiscoveryStatus}, but they have been included in the diagram for clarity.
 * These two states define the elimination path for a peer from the underlying
 * table.
 *
 * <p>If an expectation to receive a message was unmet, following the evaluation
 * of a failure condition, the peer will be physically dropped (eliminated) from
 * the table.
 */
public class PeerDiscoveryController {
  ...
  private final Map<Bytes, PeerInteractionState> inflightInteractions =
  ...
  private RetryDelayFunction retryDelayFunction =
      RetryDelayFunction.linear(1.5, 2000, 60000);
  ...
  private PeerDiscoveryController(
      final NodeKey nodeKey, final DiscoveryPeer localPeer,
      final PeerTable peerTable, final Collection<DiscoveryPeer> bootstrapNodes,
      final OutboundMessageHandler outboundMessageHandler,
      final TimerUtil timerUtil, final AsyncExecutor workerExecutor,
      final long tableRefreshIntervalMs, final long cleanPeerTableIntervalMs,
    ...
    this.peerPermissions =
        new PeerDiscoveryPermissions(localPeer, peerPermissions);

    metricsSystem.createIntegerGauge(
        BesuMetricCategory.NETWORK, "discovery_inflight_interactions_current",
        "Current number of inflight discovery interactions",
        inflightInteractions::size);

    interactionCounter = metricsSystem.createLabelledCounter(
        BesuMetricCategory.NETWORK, "discovery_interaction_count",
        "Total number of discovery interactions initiated", "type");

    interactionRetryCounter = metricsSystem.createLabelledCounter(
        BesuMetricCategory.NETWORK, "discovery_interaction_retry_count",
        "Total number of interaction retries performed", "type");
  }
  ...
  public void start() {
    if (!started.compareAndSet(false, true)) {
      throw new IllegalStateException(
          "The peer table had already been started");
    }
    ...
    recursivePeerRefreshState = new RecursivePeerRefreshState(
        this::bond, this::findNodes, timerUtil, localPeer, peerTable,
        peerPermissions, PEER_REFRESH_ROUND_TIMEOUT_IN_SECONDS, 100);
    ...
    final long refreshTimerId = timerUtil.setPeriodic(
    ...
    cleanTableTimerId = OptionalLong.of(timerUtil.setPeriodic(
        cleanPeerTableIntervalMs, this::cleanPeerTableIfRequired));
  }
  ...
  private void
  handlePermissionsUpdate(final boolean addRestrictions,
    ...
    // If we have an explicit list of peers, drop each peer from our discovery
    // table
  ...
  public void dropPeer(final PeerId peer) { peerTable.tryEvict(peer); }

  /**
   * Handles an incoming message and processes it based on the state machine for
   * the {@link DiscoveryPeer}.
   *
   * <p>The callback will be called with the canonical representation of the
   * sender Peer as stored in our table, or with an empty Optional if the
   * message was out of band and we didn't process it.
   *
  ...
  public void onMessage(final Packet packet, final DiscoveryPeer sender) {
    ...
    final Optional<DiscoveryPeer> maybeKnownPeer = peerTable.get(sender).filter(
        known -> known.discoveryEndpointMatches(sender));
    ...
    case PING:
      if (peerPermissions.allowInboundBonding(peer)) {
        ...
        final PingPacketData ping =
            packet.getPacketData(PingPacketData.class).get();
        respondToPing(ping, packet.getHash(), peer);
      }
      break;
    case PONG:
      matchInteraction(packet).ifPresent(interaction -> {
        addToPeerTable(peer);
        recursivePeerRefreshState.onBondingComplete(peer);
      });
      break;
    case NEIGHBORS:
      matchInteraction(packet).ifPresent(
          interaction
  ...
  private boolean addToPeerTable(final DiscoveryPeer peer) {
    ...
    if (result.getOutcome() == PeerTable.AddResult.AddOutcome.ALREADY_EXISTED) {
      ...
      peerTable.tryAdd(peer);
               ...
               PeerTable.AddResult.AddOutcome.BUCKET_FULL) {
  ...
  private Optional<PeerInteractionState> matchInteraction(final Packet packet) {
    final PeerInteractionState interaction =
        inflightInteractions.get(packet.getNodeId());
  ...
  /**
   * Refreshes the peer table by generating a random ID and interrogating the
   * closest nodes for it. Currently the refresh process is NOT recursive.
   */
  private void refreshTable() {
    final Bytes target = Peer.randomId();
    final List<DiscoveryPeer> initialPeers =
        peerTable.nearestPeers(Peer.randomId(), 16);
  ...
  void bond(final DiscoveryPeer peer) {
    ...
    final Consumer<PeerInteractionState> action = interaction -> {
      ...
      createPacket(PacketType.PING, data, pingPacket -> {
        final Bytes pingHash = pingPacket.getHash();
        // Update the matching filter to only accept the PONG if it echoes the
        // hash of our PING.
        final Predicate<Packet> newFilter = packet
            -> packet.getPacketData(PongPacketData.class)
    ...
    final PeerInteractionState ping = new PeerInteractionState(
        action, peer.getId(), PacketType.PONG, (packet) -> false, true);
    dispatchInteraction(peer, ping);
  }

  private void sendPacket(final DiscoveryPeer peer, final PacketType type,
                          final PacketData data) {
    createPacket(type, data, packet -> {
  ...
  @VisibleForTesting
  void createPacket(final PacketType type, final PacketData data,
                    final Consumer<Packet> handler) {
    // Creating packets is quite expensive because they have to be
    // cryptographically signed So ensure the work is done on a worker thread to
    // avoid blocking the vertx event thread.
    workerExecutor.execute(() -> Packet.create(type, data, nodeKey))
        .thenAccept(handler)
        .exceptionally(error -> {
  ...
  private void findNodes(final DiscoveryPeer peer, final Bytes target) {
    final Consumer<PeerInteractionState> action = (interaction) -> {
      final FindNeighborsPacketData data =
          FindNeighborsPacketData.create(target);
      sendPacket(peer, PacketType.FIND_NEIGHBORS, data);
    };
    final PeerInteractionState interaction = new PeerInteractionState(
        action, peer.getId(), PacketType.NEIGHBORS, packet -> true, true);
   ...
   *
   * <p>If a previous inflightInteractions interaction existed, we cancel any
   * associated timers.
   *
   * @param peer The peer.
   * @param state The state.
   */
  private void dispatchInteraction(final Peer peer,
                                   final PeerInteractionState state) {
    final PeerInteractionState previous =
        inflightInteractions.put(peer.getId(), state);
    if (previous != null) {
  ...
  private void respondToPing(final PingPacketData packetData,
                             final Bytes pingHash, final DiscoveryPeer sender) {
    if (packetData.getExpiration() < Instant.now().getEpochSecond()) {
      return;
    }
    final PongPacketData data =
        PongPacketData.create(packetData.getFrom(), pingHash);
    sendPacket(sender, PacketType.PONG, data);
  }

  private void respondToFindNeighbors(final FindNeighborsPacketData packetData,
    ...
    final List<DiscoveryPeer> peers =
        peerTable.nearestPeers(packetData.getTarget(), 16);
  ...
  private void
  dispatchPeerBondedEvent(final Subscribers<PeerBondedObserver> observers,
                          final PeerDiscoveryEvent.PeerBondedEvent event) {
    observers.forEach(observer -> observer.onPeerBonded(event));
  }

  /**
   * Returns a copy of the known peers. Modifications to the list will not
   * update the table's state, but modifications to the Peers themselves will.
   *
   * @return List of peers.
   */
  public Stream<DiscoveryPeer> streamDiscoveredPeers() {
    return peerTable.streamAllPeers().filter(
        peerPermissions::isAllowedInPeerTable);
  }

  public void
  setRetryDelayFunction(final RetryDelayFunction retryDelayFunction) {
  ...
  private class PeerInteractionState implements Predicate<Packet> {
    ...
    /**
     * The action that led to the peer being in this state (e.g. sending a PING
     * or NEIGHBORS message), in case it needs to be retried.
     */
    private final Consumer<PeerInteractionState> action;

    private final Bytes peerId;
    /**
     * The expected type of the message that will transition the peer out of
     * this state.
     */
    ...
    PeerInteractionState(final Consumer<PeerInteractionState> action,
                         final Bytes peerId, final PacketType expectedType,
    ...
    public boolean test(final Packet packet) {
      return expectedType == packet.getType() &&
          (filter == null || filter.test(packet));
    }

    void updateFilter(final Predicate<Packet> filter) { this.filter = filter; }

    /**
     * Executes the action associated with this state. Sets a "boomerang" timer
     * to itself in case the action is retryable.
     *
     * @param lastTimeout the previous timeout, or 0 if this is the first time
    ...
    void execute(final long lastTimeout, final int retryCount) {
      ...
      if (retryable && retryCount < MAX_RETRIES) {
        ...
        timerId = OptionalLong.of(timerUtil.setTimer(newTimeout, () -> {
          retryCounter.inc();
    ...
    void cancelTimers() { timerId.ifPresent(timerUtil::cancelTimer); }
  }
  ...
  public static class Builder {
    ...
    private long tableRefreshIntervalMs =
        MILLISECONDS.convert(30, TimeUnit.MINUTES);
    private long cleanPeerTableIntervalMs =
        MILLISECONDS.convert(1, TimeUnit.MINUTES);
    private final List<DiscoveryPeer> bootstrapNodes = new ArrayList<>();
    private PeerTable peerTable;
    private Subscribers<PeerBondedObserver> peerBondedObservers =
        Subscribers.create();
    ...
    public PeerDiscoveryController build() {
      ...
      if (peerTable == null) {
        peerTable =
            new PeerTable(this.nodeKey.getPublicKey().getEncodedBytes(), 16);
      }

      return new PeerDiscoveryController(
          nodeKey, localPeer, peerTable, bootstrapNodes, outboundMessageHandler,
          timerUtil, workerExecutor, tableRefreshIntervalMs,
          cleanPeerTableIntervalMs, peerRequirement, peerPermissions,
          peerBondedObservers, metricsSystem);
    }
    ...
    private void validateRequiredDependency(final Object object,
                                            final String name) {
    ...
    public Builder
    bootstrapNodes(final Collection<DiscoveryPeer> bootstrapNodes) {
      this.bootstrapNodes.addAll(bootstrapNodes);
      return this;
    }

    public Builder outboundMessageHandler(
        final OutboundMessageHandler outboundMessageHandler) {
    ...
    public Builder
    cleanPeerTableIntervalMs(final long cleanPeerTableIntervalMs) {
