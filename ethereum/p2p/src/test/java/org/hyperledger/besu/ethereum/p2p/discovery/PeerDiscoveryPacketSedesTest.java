...
public class PeerDiscoveryPacketSedesTest {
  ...
  public void serializeDeserializeEntirePacket() {
    ...
    final Packet packet =
        Packet.create(PacketType.FIND_NEIGHBORS, packetData, nodeKey);
    ...
    assertThat(decoded.getNodeId())
        .isEqualTo(nodeKey.getPublicKey().getEncodedBytes());
    assertThat(decoded.getPacketData(NeighborsPacketData.class)).isNotPresent();
    assertThat(decoded.getPacketData(FindNeighborsPacketData.class))
        .isPresent();
  }
  ...
  public void serializeDeserializeFindNeighborsPacketData() {
    ...
    // Fuzziness: allow a skew of 2 seconds between the time the message was
    // generated until the assertion.
    assertThat(deserialized.getExpiration())
        .isCloseTo(PacketData.defaultExpiration(), offset(2L));
  }
  ...
  public void integrityCheckFailsUnmatchedHash() {
    ...
    final Packet packet =
        Packet.create(PacketType.FIND_NEIGHBORS, data, nodeKey);
    ...
    // Change one bit in the last byte, which belongs to the payload, hence the
    // hash will not match any longer.
