...
public class DetermineCommonAncestorTaskTest {
  ...
  private final BlockDataGenerator blockDataGenerator =
  ...
  public void shouldFailIfPeerDisconnects() {
    final Block block =
        blockDataGenerator.nextBlock(localBlockchain.getChainHeadBlock());
    ...
    final EthTask<BlockHeader> task = DetermineCommonAncestorTask.create(
        ...
        respondingEthPeer.getEthPeer(), defaultHeaderRequestSize,
        metricsSystem);

    final AtomicReference<Throwable> failure = new AtomicReference<>();
    final CompletableFuture<BlockHeader> future = task.run();
    future.whenComplete((response, error) -> { failure.set(error); });
  ...
  public void shouldHandleEmptyResponses() {
    ...
    final RespondingEthPeer.Responder emptyResponder =
  ...
  public void calculateSkipInterval() {
    ...
    final int skipInterval = DetermineCommonAncestorTask.calculateSkipInterval(
        range, headerRequestSize);
    final int count =
        DetermineCommonAncestorTask.calculateCount(range, skipInterval);
  ...
  public void shouldIssueConsistentNumberOfRequestsToPeer() {
    ...
    final DetermineCommonAncestorTask task = DetermineCommonAncestorTask.create(
        protocolSchedule, protocolContext, ethContext,
        respondingEthPeer.getEthPeer(), defaultHeaderRequestSize,
        metricsSystem);
    ...
    future.whenComplete((response, error) -> { result.set(response); });

    Assertions.assertThat(result.get().getHash())
        .isEqualTo(MainnetBlockHeaderFunctions.createHash(
            localGenesisBlock.getHeader()));
  ...
  public void shouldShortCircuitOnHeaderInInitialRequest() {
    ...
    final DetermineCommonAncestorTask task = DetermineCommonAncestorTask.create(
        protocolSchedule, protocolContext, ethContext,
        respondingEthPeer.getEthPeer(), 10, metricsSystem);
    final DetermineCommonAncestorTask spy = spy(task);
  ...
  public void returnsImmediatelyWhenThereIsNoWorkToDo() throws Exception {
    ...
    final EthTask<BlockHeader> task = DetermineCommonAncestorTask.create(
        protocolSchedule, protocolContext, ethContext, peer,
        defaultHeaderRequestSize, metricsSystem);
    ...
    verify(peer, times(0))
        .getHeadersByHash(any(), anyInt(), anyInt(), anyBoolean());
    verify(peer, times(0))
        .getHeadersByNumber(anyLong(), anyInt(), anyInt(), anyBoolean());
    verify(peer, times(0)).send(any());
  }

  /**
   * @param localBlockCount The number of local blocks to create. Highest block
   *     will be: {@code localBlockHeight} - 1.
   * @param remoteBlockCount The number of remote blocks to create. Highest
   *     block will be: {@code remoteBlockCount} - 1.
   * @param blocksInCommon The number of blocks shared between local and remote.
   *     If a common ancestor exists, its block number will be: {@code
   *     blocksInCommon} - 1
   * @return
   */
  private Blockchain setupLocalAndRemoteChains(final int localBlockCount,
                                               final int remoteBlockCount,
                                               final int blocksInCommon) {
    checkArgument(localBlockCount >= 1);
    checkArgument(remoteBlockCount >= 1);
    checkArgument(blocksInCommon >= 0);
    checkArgument(blocksInCommon <=
                  Math.min(localBlockCount, remoteBlockCount));

    final Block remoteGenesis = (blocksInCommon > 0)
                                    ? localGenesisBlock
                                    : blockDataGenerator.genesisBlock();
    ...
    if (blocksInCommon > 1) {
      ...
      for (Block commonBlock : commonBlocks) {
        List<TransactionReceipt> receipts =
            blockDataGenerator.receipts(commonBlock);
    ...
    if (localBlockCount > blocksInCommon) {
      ...
      final int currentHeight =
          Math.toIntExact(localBlockchain.getChainHeadBlockNumber() -
                          BlockHeader.GENESIS_BLOCK_NUMBER + 1);
      List<Block> localBlocks = blockDataGenerator.blockSequence(
          localChainHead, localBlockCount - currentHeight);
    ...
    if (remoteBlockCount > blocksInCommon) {
      ...
      final int currentHeight =
          Math.toIntExact(remoteChain.getChainHeadBlockNumber() -
                          BlockHeader.GENESIS_BLOCK_NUMBER + 1);
      List<Block> remoteBlocks = blockDataGenerator.blockSequence(
          remoteChainHead, remoteBlockCount - currentHeight);
