...
public class TransactionPoolFactoryTest {
  ...
  public void testDisconnect() {
    ...
    when(blockchain.getBlockHashByNumber(anyLong()))
        .thenReturn(Optional.of(mock(Hash.class)));
    when(context.getBlockchain()).thenReturn(blockchain);
    final EthPeers ethPeers =
        new EthPeers("ETH", TestClock.fixed(), new NoOpMetricsSystem());
    ...
    final TransactionPool pool = TransactionPoolFactory.createTransactionPool(
        schedule, context, ethContext, new NoOpMetricsSystem(), state,
        Wei.of(1),
        new TransactionPoolConfiguration(
            1, 1, 1, 1, TransactionPoolConfiguration.DEFAULT_PRICE_BUMP),
        pendingTransactions, peerTransactionTracker, transactionsMessageSender,
        Optional.of(peerPendingTransactionTracker),
        Optional.of(pendingTransactionsMessageSender), true, Optional.empty());

    final EthProtocolManager ethProtocolManager = new EthProtocolManager(
        blockchain, BigInteger.ONE, mock(WorldStateArchive.class), pool,
        new EthProtocolConfiguration(5, 5, 5, 5, 5, true), ethPeers,
        mock(EthMessages.class), ethContext, Collections.emptyList(), true,
        mock(EthScheduler.class), mock(ForkIdManager.class));
    ...
    verify(peerPendingTransactionTracker, times(1))
        .onDisconnect(ethPeer.getEthPeer());
  }
  ...
  public void testNoEth65() {
    ...
    when(pendingTransactions.addLocalTransaction(any()))
        .thenReturn(TransactionAddedStatus.ADDED);
    ...
    // Now that we have the mocks we try to run the methods and see if smoke
    // comes out. Only exceptions cause the test to fail.
