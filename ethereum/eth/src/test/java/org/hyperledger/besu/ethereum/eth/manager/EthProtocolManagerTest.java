...
public final class EthProtocolManagerTest {
  ...
  public void disconnectOnUnsolicitedMessage() {
    try (final EthProtocolManager ethManager =
      ...
      final MessageData messageData = BlockHeadersMessage.create(
          Collections.singletonList(blockchain.getBlockHeader(1).get()));
      final MockPeerConnection peer =
          setupPeer(ethManager, (cap, msg, conn) -> {});
      ethManager.processMessage(EthProtocol.ETH63,
                                new DefaultMessage(peer, messageData));
  ...
  public void disconnectOnWrongChainId() {
    try (final EthProtocolManager ethManager =
      ...
      final StatusMessage statusMessage = StatusMessage.create(
          EthProtocol.EthVersion.V63, BigInteger.valueOf(2222),
          blockchain.getChainHead().getTotalDifficulty(),
          blockchain.getChainHeadHash(),
          blockchain.getBlockHeader(BlockHeader.GENESIS_BLOCK_NUMBER)
              .get()
              .getHash());
      ethManager.processMessage(EthProtocol.ETH63,
                                new DefaultMessage(peer, statusMessage));
  ...
  public void disconnectOnWrongGenesisHash() {
    try (final EthProtocolManager ethManager =
      ...
      // Send status message with wrong chain
      final StatusMessage statusMessage = StatusMessage.create(
          EthProtocol.EthVersion.V63, BigInteger.ONE,
          blockchain.getChainHead().getTotalDifficulty(), gen.hash(),
          blockchain.getBlockHeader(BlockHeader.GENESIS_BLOCK_NUMBER)
              .get()
              .getHash());
      ethManager.processMessage(EthProtocol.ETH63,
  ...
  @Test
  public void respondToGetHeaders()
  ...
  @Test
  public void respondToGetHeadersWithinLimits()
      throws ExecutionException, InterruptedException {
    final CompletableFuture<Void> done = new CompletableFuture<>();
    final int limit = 5;
    try (final EthProtocolManager ethManager =
  ...
  @Test
  public void respondToGetHeadersReversed()
      throws ExecutionException, InterruptedException {
    final CompletableFuture<Void> done = new CompletableFuture<>();
    try (final EthProtocolManager ethManager =
             EthProtocolManagerTestUtil.create(
  ...
  public void respondToGetHeadersWithSkip()
    ...
    try (final EthProtocolManager ethManager =
      ...
      final PeerSendHandler onSend = (cap, message, conn) -> {
        ...
        assertThat(headers.size()).isEqualTo(blockCount);
        for (int i = 0; i < blockCount; i++) {
          assertThat(headers.get(i).getNumber())
              .isEqualTo(startBlock + i * (skip + 1));
        }
        done.complete(null);
      };
      final PeerConnection peer = setupPeer(ethManager, onSend);
  ...
  private MockPeerConnection setupPeer(final EthProtocolManager ethManager,
                                       final PeerSendHandler onSend) {
    final MockPeerConnection peer =
        setupPeerWithoutStatusExchange(ethManager, onSend);
    final StatusMessage statusMessage = StatusMessage.create(
        EthProtocol.EthVersion.V63, BigInteger.ONE,
        blockchain.getChainHead().getTotalDifficulty(),
        blockchain.getChainHeadHash(),
  ...
  setupPeerWithoutStatusExchange(final EthProtocolManager ethManager,
                                 final PeerSendHandler onSend) {
    final Set<Capability> caps =
        new HashSet<>(Collections.singletonList(EthProtocol.ETH63));
  ...
  @Test
  public void respondToGetHeadersPartial()
      throws ExecutionException, InterruptedException {
    final CompletableFuture<Void> done = new CompletableFuture<>();
    try (final EthProtocolManager ethManager =
             EthProtocolManagerTestUtil.create(
  ...
  @Test
  public void respondToGetHeadersEmpty()
      throws ExecutionException, InterruptedException {
    final CompletableFuture<Void> done = new CompletableFuture<>();
    try (final EthProtocolManager ethManager =
             EthProtocolManagerTestUtil.create(
  ...
  @Test
  public void respondToGetBodies()
      throws ExecutionException, InterruptedException {
    final CompletableFuture<Void> done = new CompletableFuture<>();
    try (final EthProtocolManager ethManager =
             EthProtocolManagerTestUtil.create(
      ...
      final PeerSendHandler onSend = (cap, message, conn) -> {
        ...
        final BlockBodiesMessage blocksMessage =
            BlockBodiesMessage.readFrom(message);
  ...
  @Test
  public void respondToGetBodiesWithinLimits()
      throws ExecutionException, InterruptedException {
    final CompletableFuture<Void> done = new CompletableFuture<>();
    final int limit = 5;
    try (final EthProtocolManager ethManager =
  ...
  @Test
  public void respondToGetBodiesPartial()
      throws ExecutionException, InterruptedException {
    final CompletableFuture<Void> done = new CompletableFuture<>();
    try (final EthProtocolManager ethManager =
             EthProtocolManagerTestUtil.create(
      ...
      final BlockHeader header =
          blockchain.getBlockHeader(expectedBlockNumber).get();
      final BlockBody body = blockchain.getBlockBody(header.getHash()).get();
      final Block expectedBlock = new Block(header, body);

      final List<Hash> hashes =
          Arrays.asList(gen.hash(), expectedBlock.getHash(), gen.hash());
      final MessageData messageData = GetBlockBodiesMessage.create(hashes);
  ...
  @Test
  public void respondToGetReceipts()
      throws ExecutionException, InterruptedException {
    final CompletableFuture<Void> done = new CompletableFuture<>();
    try (final EthProtocolManager ethManager =
             EthProtocolManagerTestUtil.create(
      ...
      final PeerSendHandler onSend = (cap, message, conn) -> {
        ...
        final ReceiptsMessage receiptsMessage =
            ReceiptsMessage.readFrom(message);
  ...
  @Test
  public void respondToGetReceiptsWithinLimits()
      throws ExecutionException, InterruptedException {
    final CompletableFuture<Void> done = new CompletableFuture<>();
    final int limit = 5;
    try (final EthProtocolManager ethManager =
  ...
  @Test
  public void respondToGetReceiptsPartial()
      throws ExecutionException, InterruptedException {
    final CompletableFuture<Void> done = new CompletableFuture<>();
    try (final EthProtocolManager ethManager =
             EthProtocolManagerTestUtil.create(
      ...
      final MessageData messageData = GetReceiptsMessage.create(
          Arrays.asList(gen.hash(), blockHash, gen.hash()));
  ...
  public void respondToGetNodeData() throws Exception {
    ...
    try (final EthProtocolManager ethManager =
      ...
      for (int i = 0; i < blockCount; i++) {
        ...
        expectedResults.add(
            worldStateArchive.getNodeData(header.getStateRoot()).get());
      }
      final MessageData messageData =
          GetNodeDataMessage.create(requestedHashes);
      ...
      final PeerSendHandler onSend = (cap, message, conn) -> {
        ...
        final NodeDataMessage receiptsMessage =
            NodeDataMessage.readFrom(message);
  ...
  public void newBlockMinedSendsNewBlockMessageToAllPeers() {
    try (final EthProtocolManager ethManager =
      ...
      final Block minedBlock =
          new Block(blockchain.getBlockHeader(chainHeadHash).get(),
      ...
      final ArgumentCaptor<Capability> capabilityCaptor =
          ArgumentCaptor.forClass(Capability.class);

      verify(onSend, times(PEER_COUNT))
          .exec(capabilityCaptor.capture(), messageSentCaptor.capture(),
                receivingPeerCaptor.capture());

      // assert that all entries in capability param were Eth63
      assertThat(capabilityCaptor.getAllValues().stream().distinct().collect(
                     Collectors.toList()))
          .isEqualTo(Collections.singletonList(EthProtocol.ETH63));

      // assert that all messages transmitted contain the expected block & total
      // difficulty.
      final ProtocolSchedule protocolSchdeule =
      ...
      for (final NewBlockMessage msg : messageSentCaptor.getAllValues()) {
        ...
        assertThat(msg.totalDifficulty(protocolSchdeule))
  ...
  public void shouldSuccessfullyRespondToGetHeadersRequestLessThanZero()
    ...
    final BlockDataGenerator.BlockOptions options =
        new BlockDataGenerator.BlockOptions().setBlockNumber(1L).setParentHash(
            blockchain.getBlockHashByNumber(0L).get());
    ...
    try (final EthProtocolManager ethManager =
      ...
      final MessageData messageData = GetBlockHeadersMessage.create(
          startBlock, requestedBlockCount, 0, true);
      final MockPeerConnection.PeerSendHandler onSend =
        ...
        for (int i = 0; i < receivedBlockCount; i++) {
          assertThat(headers.get(i).getNumber())
              .isEqualTo(receivedBlockCount - 1 - i);
        }
        done.complete(null);
      };
  ...
  public void transactionMessagesGoToTheCorrectExecutor() {
    ...
    final EthScheduler ethScheduler = new EthScheduler(
    ...
    final MessageData raw =
        new RawMessage(EthPV62.TRANSACTIONS, initialMessage.getData());
    final TransactionsMessage transactionMessage =
        TransactionsMessage.readFrom(raw);

    try (
        final EthProtocolManager ethManager = EthProtocolManagerTestUtil.create(
            blockchain, ethScheduler, protocolContext.getWorldStateArchive(),
            transactionPool, EthProtocolConfiguration.defaultConfig())) {
      // Create a transaction pool.  This has a side effect of registering a
      // listener for the transactions message.
      TransactionPoolFactory.createTransactionPool(
          protocolSchedule, protocolContext, ethManager.ethContext(),
          TestClock.fixed(), metricsSystem, mock(SyncState.class), Wei.ZERO,
          TransactionPoolConfiguration.builder().build(), true,
          Optional.empty());

      // Send just a transaction message.
      final PeerConnection peer =
          setupPeer(ethManager, (cap, msg, connection) -> {});
      ethManager.processMessage(EthProtocol.ETH63,
                                new DefaultMessage(peer, transactionMessage));

      // Verify the regular message executor and scheduled executor got nothing
      // to execute.
