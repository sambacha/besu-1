...
public class RespondingEthPeer {
  ...
  private RespondingEthPeer(
      ...
      final MockPeerConnection peerConnection, final EthPeer ethPeer,
  ...
  public static void respondOnce(final Responder responder,
  ...
  public MockPeerConnection getPeerConnection() { return peerConnection; }

  public static Builder builder() { return new Builder(); }

  private static RespondingEthPeer
  create(final EthProtocolManager ethProtocolManager, final Hash chainHeadHash,
         final Difficulty totalDifficulty, final OptionalLong estimatedHeight,
    ...
    final MockPeerConnection peerConnection = new MockPeerConnection(
        caps,
        (cap, msg,
         conn) -> outgoingMessages.add(new OutgoingMessage(cap, msg)));
    ...
    estimatedHeight.ifPresent(
        height -> peer.chainState().update(chainHeadHash, height));
    peer.registerStatusSent();

    return new RespondingEthPeer(ethProtocolManager, peerConnection, peer,
                                 outgoingMessages);
  }

  public EthPeer getEthPeer() { return ethPeer; }

  public void respondWhile(final Responder responder,
                           final RespondWhileCondition condition) {
    int counter = 0;
    while (condition.shouldRespond()) {
      respond(responder);
      counter++;
      if (counter > 10_000) {
        // Limit applied to avoid tests hanging forever which is hard to track
        // down.
  ...
  public void
  respondWhileOtherThreadsWork(final Responder responder,
                               final RespondWhileCondition condition) {
    int counter = 0;
    while (condition.shouldRespond()) {
      try {
        final OutgoingMessage message =
            outgoingMessages.poll(1, TimeUnit.SECONDS);
        if (message != null) {
          respondToMessage(responder, message);
          counter++;
          if (counter > 10_000) {
            // Limit applied to avoid tests hanging forever which is hard to
            // track down.
            throw new IllegalStateException(
                "Responded 10,000 times and stop condition still not reached.");
          }
  ...
  private void respondToMessage(final Responder responder,
  ...
  public static Responder targetedResponder(
      ...
      final BiFunction<Capability, MessageData, MessageData>
          responseGenerator) {
    return (cap, msg) -> {
  ...
  blockchainResponder(final Blockchain blockchain,
                      final WorldStateArchive worldStateArchive) {
    return blockchainResponder(blockchain, worldStateArchive,
  ...
  blockchainResponder(final Blockchain blockchain,
    ...
    return (cap, msg) -> {
      ...
      case EthPV63.GET_NODE_DATA:
        response =
            EthServer.constructGetNodeDataResponse(worldStateArchive, msg, 200);
        break;
      case EthPV65.GET_POOLED_TRANSACTIONS:
        response = EthServer.constructGetPooledTransactionsResponse(
            transactionPool, msg, 200);
      }
  ...
  public static Responder
  wrapResponderWithCollector(final Responder responder,
                             final List<MessageData> messageCollector) {
    return (cap, msg) -> {
  ...
  /**
   * Create a responder that only responds with a fixed portion of the available
   * data.
   *
  ...
  public static Responder partialResponder(
      ...
      final TransactionPool transactionPool,
      final ProtocolSchedule protocolSchedule, final float portion) {
    checkArgument(portion >= 0.0 && portion <= 1.0,
                  "Portion is in the range [0.0..1.0]");

    final Responder fullResponder =
        blockchainResponder(blockchain, worldStateArchive, transactionPool);
    return (cap, msg) -> {
      final Optional<MessageData> maybeResponse =
          fullResponder.respond(cap, msg);
      ...
      case EthPV62.GET_BLOCK_HEADERS:
        final BlockHeadersMessage headersMessage =
            BlockHeadersMessage.readFrom(originalResponse);
        ...
        break;
      case EthPV62.GET_BLOCK_BODIES:
        final BlockBodiesMessage bodiesMessage =
            BlockBodiesMessage.readFrom(originalResponse);
      ...
      case EthPV63.GET_RECEIPTS:
        final ReceiptsMessage receiptsMessage =
            ReceiptsMessage.readFrom(originalResponse);
        ...
        final List<List<TransactionReceipt>> partialReceipts =
            originalReceipts.subList(0,
                                     (int)(originalReceipts.size() * portion));
        partialResponse = ReceiptsMessage.create(partialReceipts);
        break;
      case EthPV63.GET_NODE_DATA:
        final NodeDataMessage nodeDataMessage =
            NodeDataMessage.readFrom(originalResponse);
        final List<Bytes> originalNodeData =
            Lists.newArrayList(nodeDataMessage.nodeData());
        final List<Bytes> partialNodeData = originalNodeData.subList(
            0, (int)(originalNodeData.size() * portion));
      ...
      case EthPV65.GET_POOLED_TRANSACTIONS:
        ...
        final List<Transaction> partialPooledTx = originalPooledTx.subList(
            0, (int)(originalPooledTx.size() * portion));
  ...
  public static class Builder {
    ...
    public RespondingEthPeer build() {
      ...
      return RespondingEthPeer.create(ethProtocolManager, chainHeadHash,
                                      totalDifficulty, estimatedHeight,
                                      peerValidators);
    }

    public Builder
    ethProtocolManager(final EthProtocolManager ethProtocolManager) {
  ...
  static class OutgoingMessage {
    ...
    OutgoingMessage(final Capability capability,
    ...
    public MessageData messageData() { return messageData; }
  }
  ...
  public interface RespondWhileCondition { boolean shouldRespond(); }
}
