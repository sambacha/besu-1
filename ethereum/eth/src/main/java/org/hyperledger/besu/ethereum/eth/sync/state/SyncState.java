...
public class SyncState {
  ...
  private final Map<Long, InSyncTracker> inSyncTrackers =
      new ConcurrentHashMap<>();
  private final Subscribers<SyncStatusListener> syncStatusListeners =
      Subscribers.create();
  private volatile long chainHeightListenerId;
  private volatile Optional<SyncTarget> syncTarget = Optional.empty();
  private Optional<WorldStateDownloadStatus> worldStateDownloadStatus =
      Optional.empty();
  ...
  public SyncState(final Blockchain blockchain, final EthPeers ethPeers) {
    ...
    blockchain.observeBlockAdded(event -> {
      if (event.isNewCanonicalHead()) {
        checkInSync();
      }
    });

    // Add new peer listener to prevent permissioned PoA network stalling on
    // start-up. https://github.com/hyperledger/besu/issues/528
    newPeerListenerId = Optional.of(ethPeers.subscribeConnect(newPeer -> {
   ...
   * {@code SYNC_TOLERANCE} behind the highest estimated remote chain height.
   *
   * @param listener The callback to invoke when the sync status changes
   * @return An {@code Unsubscriber} that can be used to stop listening for
   *     these events
   */
  ...
  public long subscribeInSync(final InSyncListener listener,
    ...
    final InSyncTracker inSyncTracker =
        InSyncTracker.create(listener, syncTolerance);
  ...
  public Optional<SyncStatus> syncStatus() { return syncStatus(syncTarget); }

  public Optional<SyncTarget> syncTarget() { return syncTarget; }

  public void setSyncTarget(final EthPeer peer,
  ...
  public void setWorldStateDownloadStatus(
      final WorldStateDownloadStatus worldStateDownloadStatus) {
    this.worldStateDownloadStatus =
        Optional.ofNullable(worldStateDownloadStatus);
  }
  ...
  public boolean isInSync(final long syncTolerance) {
    return isInSync(getLocalChainHead(), getSyncTargetChainHead(),
                    getBestPeerChainHead(), syncTolerance);
  }

  private boolean isInSync(final ChainHead localChain,
                           final Optional<ChainHeadEstimate> syncTargetChain,
                           final Optional<ChainHeadEstimate> bestPeerChain,
                           final long syncTolerance) {
    // Sync target may be temporarily empty while we switch sync targets during
    // a sync, so check both the sync target and our best peer to determine if
    // we're in sync or not
  ...
  private boolean isInSync(final ChainHead localChain,
    ...
    return remoteChain
        .map(remoteState
             -> InSyncTracker.isInSync(localChain, remoteState, syncTolerance))
        .orElse(true);
  }

  private ChainHead getLocalChainHead() { return blockchain.getChainHead(); }
  ...
  private Optional<ChainHeadEstimate> getBestPeerChainHead() {
    return ethPeers.bestPeerWithHeightEstimate().map(
  ...
  public void clearSyncTarget() { replaceSyncTarget(Optional.empty()); }

  private synchronized void
  replaceSyncTarget(final Optional<SyncTarget> newTarget) {
  ...
  private Optional<SyncStatus>
  syncStatus(final Optional<SyncTarget> maybeTarget) {
    return maybeTarget.map(target -> {
      ...
      return new DefaultSyncStatus(
          commonAncestor, chainHeadBlockNumber, highestKnownBlock,
          worldStateDownloadStatus.flatMap(
              WorldStateDownloadStatus::getPulledStates),
          worldStateDownloadStatus.flatMap(
              WorldStateDownloadStatus::getKnownStates));
    });
  }
  ...
  private void addEstimatedHeightListener(final SyncTarget target) {
    chainHeightListenerId = target.addPeerChainEstimatedHeightListener(
        estimatedHeight -> checkInSync());
  }
  ...
  public long bestChainHeight(final long localChainHeight) {
    return Math.max(localChainHeight,
                    ethPeers.bestPeerWithHeightEstimate()
  ...
  private synchronized void checkInSync() {
    ...
    inSyncTrackers.values().forEach(
        (syncTracker)
            -> syncTracker.checkState(localChain, syncTargetChain,
                                      bestPeerChain));
  }
}
