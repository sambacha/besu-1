...
public class FastSyncActions {
  ...
  public FastSyncActions(final SynchronizerConfiguration syncConfig,
    ...
    pivotBlockSelectionCounter = metricsSystem.createCounter(
        BesuMetricCategory.SYNCHRONIZER, "fast_sync_pivot_block_selected_count",
        "Number of times a fast sync pivot block has been selected");
    metricsSystem.createLongGauge(
        BesuMetricCategory.SYNCHRONIZER, "fast_sync_pivot_block_current",
        "The current fast sync pivot block", pivotBlockGauge::get);
  }

  public CompletableFuture<FastSyncState>
  waitForSuitablePeers(final FastSyncState fastSyncState) {
    if (fastSyncState.hasPivotBlockHeader()) {
      return waitForAnyPeer().thenApply(ignore -> fastSyncState);
    }

    LOG.debug("Waiting for at least {} peers.",
  ...
  scheduleFutureTask(final Supplier<CompletableFuture<T>> future,
  ...
  private CompletableFuture<Void> waitForAnyPeer() {
    final CompletableFuture<Void> waitForPeerResult =
        ethContext.getScheduler().timeout(
            WaitForPeersTask.create(ethContext, 1, metricsSystem));
    return exceptionallyCompose(waitForPeerResult, throwable -> {
  ...
  public CompletableFuture<FastSyncState>
  selectPivotBlock(final FastSyncState fastSyncState) {
    return fastSyncState.hasPivotBlockHeader() ? completedFuture(fastSyncState)
  ...
  private CompletableFuture<FastSyncState> selectPivotBlockFromPeers() {
    return ethContext.getEthPeers()
        ...
        // Only select a pivot block number when we have a minimum number of
        // height estimates
        .filter(peer -> {
        ...
        .map(peer -> {
          final long pivotBlockNumber = peer.chainState().getEstimatedHeight() -
                                        syncConfig.getFastSyncPivotDistance();
          if (pivotBlockNumber <= BlockHeader.GENESIS_BLOCK_NUMBER) {
            // Peer's chain isn't long enough, return an empty value so we can
            // try again.
            LOG.info("Waiting for peers with sufficient chain height");
            return null;
          }
          LOG.info("Selecting block number {} as fast sync pivot block.",
                   pivotBlockNumber);
  ...
  private CompletableFuture<FastSyncState> retrySelectPivotBlockAfterDelay() {
    return ethContext.getScheduler().scheduleFutureTask(
        ()
  ...
  createChainDownloader(final FastSyncState currentState) {
    return FastSyncChainDownloader.create(
        syncConfig, protocolSchedule, protocolContext, ethContext, syncState,
        metricsSystem, currentState.getPivotBlockHeader().get());
  }
}
