...
class FastSyncTargetManager extends SyncTargetManager {
  ...
  public FastSyncTargetManager(final SynchronizerConfiguration config,
  ...
  @Override
  protected CompletableFuture<Optional<EthPeer>>
  selectBestAvailableSyncTarget() {
    final Optional<EthPeer> maybeBestPeer =
        ethContext.getEthPeers().bestPeerWithHeightEstimate();
    if (!maybeBestPeer.isPresent()) {
      LOG.info("No sync target, waiting for peers: {}",
  ...
  private CompletableFuture<Optional<EthPeer>>
  confirmPivotBlockHeader(final EthPeer bestPeer) {
    final RetryingGetHeaderFromPeerByNumberTask task =
        RetryingGetHeaderFromPeerByNumberTask.forSingleNumber(
            ...
            pivotBlockHeader.getNumber(), MAX_QUERY_RETRIES_PER_PEER);
        ...
        .thenApply(result -> {
          if (peerHasDifferentPivotBlock(result)) {
            LOG.warn(
                ...
                pivotBlockHeader.getNumber(), pivotBlockHeader.getHash(),
                result.size() == 1 ? result.get(0).getHash()
                                   : "invalid response",
                bestPeer);
  ...
  public boolean shouldContinueDownloading() {
    return !protocolContext.getBlockchain().getChainHeadHash().equals(
