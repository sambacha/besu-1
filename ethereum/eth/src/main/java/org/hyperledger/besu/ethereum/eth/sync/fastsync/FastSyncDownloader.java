...
public class FastSyncDownloader {
  ...
  private volatile Optional<TrailingPeerRequirements> trailingPeerRequirements =
      Optional.empty();
  ...
  private CompletableFuture<FastSyncState>
  start(final FastSyncState fastSyncState) {
  ...
  public void stop() {
    synchronized (this) {
      if (running.compareAndSet(true, false)) {
        // Cancelling the world state download will also cause the chain
  ...
  public void deleteFastSyncState() {
    // Make sure downloader is stopped before we start cleaning up its
    // dependencies
    worldStateDownloader.cancel();
    try {
      taskCollection.close();
      if (fastSyncDataDirectory.toFile().exists()) {
        // Clean up this data for now (until fast sync resume functionality is
        // in place)
        MoreFiles.deleteRecursively(fastSyncDataDirectory,
  ...
  private FastSyncState updateMaxTrailingPeers(final FastSyncState state) {
    if (state.getPivotBlockNumber().isPresent()) {
      trailingPeerRequirements = Optional.of(new TrailingPeerRequirements(
          state.getPivotBlockNumber().getAsLong(), 0));
    } else {
      trailingPeerRequirements = Optional.empty();
    }
    return state;
  ...
  private CompletableFuture<FastSyncState>
  downloadChainAndWorldState(final FastSyncState currentState) {
    // Synchronized ensures that stop isn't called while we're in the process of
    // starting a world state and chain download. If it did we might wind up
    // starting a new download after the stop method had called cancel.
    synchronized (this) {
      if (!running.get()) {
  ...
  public Optional<TrailingPeerRequirements>
