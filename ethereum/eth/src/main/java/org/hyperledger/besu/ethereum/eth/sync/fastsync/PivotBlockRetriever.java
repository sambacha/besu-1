...
/**
 * This task attempts to find a non-controversial pivot block by confirming the
 * pivot block number with a minimum number of peers. If a particular pivot
 * block cannot be confirmed, the pivot block number is pushed back and the
 * confirmation process repeats. If a maximum number of retries is reached, the
 * task fails with a {@code FastSyncException} containing {@code
...
public class PivotBlockRetriever {
  ...
  // The max times to push the pivot block number back when peers can't agree on
  // a pivot
  ...
  private final Map<Long, PivotBlockConfirmer> confirmationTasks =
      new ConcurrentHashMap<>();

  private final AtomicBoolean isStarted = new AtomicBoolean(false);

  PivotBlockRetriever(final ProtocolSchedule protocolSchedule,
                      final EthContext ethContext,
                      final MetricsSystem metricsSystem,
                      final long pivotBlockNumber, final int peersToQuery,
  ...
  public PivotBlockRetriever(final ProtocolSchedule protocolSchedule,
                             final EthContext ethContext,
                             final MetricsSystem metricsSystem,
                             final long pivotBlockNumber,
                             final int peersToQuery,
                             final long pivotBlockNumberResetDelta) {
    this(protocolSchedule, ethContext, metricsSystem, pivotBlockNumber,
         peersToQuery, pivotBlockNumberResetDelta,
  ...
  public CompletableFuture<FastSyncState> downloadPivotBlockHeader() {
    if (isStarted.compareAndSet(false, true)) {
      LOG.info(
          "Retrieve a pivot block that can be confirmed by at least {} peers.",
          peersToQuery);
  ...
  private void confirmBlock(final long blockNumber) {
    final PivotBlockConfirmer pivotBlockConfirmationTask =
        new PivotBlockConfirmer(protocolSchedule, ethContext, metricsSystem,
                                pivotBlockNumber.get(), peersToQuery,
    ...
    pivotBlockConfirmationTask.confirmPivotBlock().whenComplete(
        this::handleConfirmationResult);
  }

  private void handleConfirmationResult(final FastSyncState fastSyncState,
                                        final Throwable error) {
    if (error != null) {
      final Throwable rootCause = ExceptionUtils.rootCause(error);
      if (rootCause instanceof
          PivotBlockConfirmer.ContestedPivotBlockException) {
        final long blockNumber =
            ((PivotBlockConfirmer.ContestedPivotBlockException)error)
                .getBlockNumber();
  ...
  private void handleContestedPivotBlock(final long contestedBlockNumber) {
    if (pivotBlockNumber.compareAndSet(contestedBlockNumber,
                                       contestedBlockNumber -
                                           pivotBlockNumberResetDelta)) {
      LOG.info("Received conflicting pivot blocks for {}.",
               contestedBlockNumber);

      final int retryCount = confirmationTasks.size();

      if ((retryCount % SUSPICIOUS_NUMBER_OF_RETRIES) == 0) {
        LOG.warn("{} attempts have failed to find a fast sync pivot block",
                 retryCount);
      }
