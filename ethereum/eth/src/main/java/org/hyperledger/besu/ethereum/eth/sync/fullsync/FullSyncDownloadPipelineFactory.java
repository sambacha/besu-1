...
public class FullSyncDownloadPipelineFactory
  ...
  createDownloadPipelineForSyncTarget(final SyncTarget target) {
    ...
    final CheckpointRangeSource checkpointRangeSource =
        new CheckpointRangeSource(
            new CheckpointHeaderFetcher(syncConfig, protocolSchedule,
                                        ethContext, Optional.empty(),
                                        metricsSystem),
    ...
    final ExtractTxSignaturesStep extractTxSignaturesStep =
        new ExtractTxSignaturesStep();
    ...
    return PipelineBuilder
        ...
        .thenProcessAsyncOrdered("downloadBodies", downloadBodiesStep,
                                 downloaderParallelism)
        .thenFlatMap("extractTxSignatures", extractTxSignaturesStep,
                     singleHeaderBufferSize)
  ...
  shouldContinueDownloadingFromPeer(final EthPeer peer,
    ...
    final boolean caughtUpToPeer = peer.chainState().getEstimatedHeight() <=
                                   lastCheckpointHeader.getNumber();
    return !peer.isDisconnected() && !caughtUpToPeer &&
