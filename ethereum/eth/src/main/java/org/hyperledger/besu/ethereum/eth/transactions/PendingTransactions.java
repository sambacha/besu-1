 ...
 * them based on priority for mining or look-up by hash.
 *
...
public class PendingTransactions {
  ...
  private final Map<Hash, TransactionInfo> pendingTransactions =
      new ConcurrentHashMap<>();
  private final SortedSet<TransactionInfo> prioritizedTransactions =
      new TreeSet<>(comparing(TransactionInfo::isReceivedFromLocalSource)
                        .thenComparing(TransactionInfo::getSequence)
                        .reversed());
  private final Map<Address, TransactionsForSenderInfo> transactionsBySender =
      new ConcurrentHashMap<>();

  private final Subscribers<PendingTransactionListener>
      pendingTransactionSubscribers = Subscribers.create();

  private final Subscribers<PendingTransactionDroppedListener>
      transactionDroppedListeners = Subscribers.create();
  ...
  public PendingTransactions(
      final int maxTransactionRetentionHours, final int maxPendingTransactions,
      final int maxPooledTransactionHashes, final Clock clock,
    ...
    this.transactionReplacementHandler =
        new TransactionPoolReplacementHandler(eip1559, priceBump);
    final LabelledMetric<Counter> transactionAddedCounter =
        metricsSystem.createLabelledCounter(
            BesuMetricCategory.TRANSACTION_POOL, "transactions_added_total",
            "Count of transactions added to the transaction pool", "source");
    ...
    transactionRemovedCounter = metricsSystem.createLabelledCounter(
        BesuMetricCategory.TRANSACTION_POOL, "transactions_removed_total",
        "Count of transactions removed from the transaction pool", "source",
        "operation");
  }
  ...
  public void evictOldTransactions() {
    ...
    pendingTransactions.values()
        .stream()
        .filter(transaction
                -> transaction.getAddedToPoolAt().isBefore(
                    removeTransactionsBefore))
        .forEach(
            transaction -> removeTransaction(transaction.getTransaction()));
  }
  ...
  @VisibleForTesting
  public TransactionAddedStatus
  addLocalTransaction(final Transaction transaction) {
  ...
  private void doRemoveTransaction(final Transaction transaction,
                                   final boolean addedToBlock) {
  ...
  private void
  incrementTransactionRemovedCounter(final boolean receivedFromLocalSource,
                                     final boolean addedToBlock) {
  ...
  public void selectTransactions(final TransactionSelector selector) {
    synchronized (prioritizedTransactions) {
      ...
      final Map<Address, AccountTransactionOrder> accountTransactions =
          new HashMap<>();
        ...
        for (final Transaction transactionToProcess :
          ...
          default:
            throw new RuntimeException(
                "Illegal value for TransactionSelectionResult.");
          }
        }
      }
      transactionsToRemove.forEach(this::removeTransaction);
  ...
  private AccountTransactionOrder
  createSenderTransactionOrder(final Address address) {
    return new AccountTransactionOrder(
        transactionsBySender.get(address)
  ...
  private TransactionAddedStatus
  addTransaction(final TransactionInfo transactionInfo) {
  ...
  private TransactionAddedStatus
  addTransactionForSenderAndNonce(final TransactionInfo transactionInfo) {
    ...
    if (existingTransaction != null) {
      if (!transactionReplacementHandler.shouldReplace(
              existingTransaction, transactionInfo,
              chainHeadHeaderSupplier.get())) {
  ...
  private void
  trackTransactionBySenderAndNonce(final TransactionInfo transactionInfo) {
    final TransactionsForSenderInfo transactionsForSenderInfo =
        transactionsBySender.computeIfAbsent(
            transactionInfo.getSender(),
            key -> new TransactionsForSenderInfo());
    transactionsForSenderInfo.addTransactionToTrack(transactionInfo.getNonce(),
                                                    transactionInfo);
  }

  private void
  removeTransactionTrackedBySenderAndNonce(final Transaction transaction) {
    Optional.ofNullable(transactionsBySender.get(transaction.getSender()))
        .ifPresent(transactionsForSender -> {
          transactionsForSender.getTransactionsInfos().remove(
  ...
  getTrackedTransactionBySenderAndNonce(final TransactionInfo transactionInfo) {
    final TransactionsForSenderInfo transactionsForSenderInfo =
        transactionsBySender.computeIfAbsent(
            transactionInfo.getSender(),
            key -> new TransactionsForSenderInfo());
    return transactionsForSenderInfo.getTransactionsInfos().get(
        transactionInfo.getNonce());
  }

  private void notifyTransactionAdded(final Transaction transaction) {
    pendingTransactionSubscribers.forEach(
        listener -> listener.onTransactionAdded(transaction));
  }

  private void notifyTransactionDropped(final Transaction transaction) {
    transactionDroppedListeners.forEach(
        listener -> listener.onTransactionDropped(transaction));
  }

  public long maxSize() { return maxPendingTransactions; }

  public int size() { return pendingTransactions.size(); }
  ...
  long subscribeDroppedTransactions(
      final PendingTransactionDroppedListener listener) {
  ...
  public OptionalLong getNextNonceForSender(final Address sender) {
    ...
    if (transactionsForSenderInfo == null ||
      ...
      final OptionalLong maybeNextGap =
          transactionsForSenderInfo.maybeNextGap();
      return maybeNextGap.isEmpty()
          ? OptionalLong.of(
                transactionsForSenderInfo.getTransactionsInfos().lastKey() + 1)
          : maybeNextGap;
    }
  }

  public void tryEvictTransactionHash(final Hash hash) {
    synchronized (newPooledHashes) { newPooledHashes.remove(hash); }
  }

  Collection<Hash> getNewPooledHashes() { return newPooledHashes; }

  /**
   * Tracks the additional metadata associated with transactions to enable
   * prioritization for mining and deciding which transactions to drop when the
   * transaction pool reaches its size limit.
   */
  public static class TransactionInfo {
    ...
    private final long
        sequence; // Allows prioritization based on order transactions are added
    ...
    public long getNonce() { return transaction.getNonce(); }

    public Address getSender() { return transaction.getSender(); }

    public boolean isReceivedFromLocalSource() {
      return receivedFromLocalSource;
    }

    public Hash getHash() { return transaction.getHash(); }

    public Instant getAddedToPoolAt() { return addedToPoolAt; }
  }
  ...
  public enum TransactionAddedStatus {
    ...
    TransactionAddedStatus() { this.invalidReason = Optional.empty(); }
