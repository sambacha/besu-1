...
class FullSyncTargetManager extends SyncTargetManager {
  ...
  FullSyncTargetManager(final SynchronizerConfiguration config,
  ...
  finalizeSelectedSyncTarget(final SyncTarget syncTarget) {
    ...
    if (protocolContext.getWorldStateArchive().isWorldStateAvailable(
            commonAncestor.getStateRoot())) {
      return Optional.of(syncTarget);
    } else {
      LOG.warn(
          "Disconnecting {} because world state is not available at common ancestor at block {}",
          syncTarget.peer(), commonAncestor.getNumber());
  ...
  selectBestAvailableSyncTarget() {
    ...
    if (!maybeBestPeer.isPresent()) {
      ...
      if (isSyncTargetReached(bestPeer)) {
        ...
        LOG.debug("Caught up to best peer: {}, Peers: {}",
