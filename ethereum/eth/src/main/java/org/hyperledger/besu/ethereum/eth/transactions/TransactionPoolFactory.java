...
public class TransactionPoolFactory {
  ...
  public static TransactionPool createTransactionPool(
      ...
      final SyncState syncState, final Wei minTransactionGasPrice,
      final TransactionPoolConfiguration transactionPoolConfiguration,
      final boolean eth65Enabled, final Optional<EIP1559> eip1559) {
    ...
    final PendingTransactions pendingTransactions = new PendingTransactions(
        ...
        transactionPoolConfiguration.getPooledTransactionHashesSize(), clock,
        metricsSystem, protocolContext.getBlockchain()::getChainHeadHeader,
        eip1559, transactionPoolConfiguration.getPriceBump());
    ...
    final Optional<PeerPendingTransactionTracker> pendingTransactionTracker =
        eth65Enabled ? Optional.of(new PeerPendingTransactionTracker(
                           pendingTransactions))
                     : Optional.empty();
    final Optional<PendingTransactionsMessageSender>
        pendingTransactionsMessageSender =
            eth65Enabled ? Optional.of(new PendingTransactionsMessageSender(
                               pendingTransactionTracker.get()))
                         : Optional.empty();

    return createTransactionPool(
        protocolSchedule, protocolContext, ethContext, metricsSystem, syncState,
        minTransactionGasPrice, transactionPoolConfiguration,
        pendingTransactions, transactionTracker, transactionsMessageSender,
        pendingTransactionTracker, pendingTransactionsMessageSender,
        eth65Enabled, eip1559);
  }

  static TransactionPool createTransactionPool(
      final ProtocolSchedule protocolSchedule,
      final ProtocolContext protocolContext, final EthContext ethContext,
      final MetricsSystem metricsSystem, final SyncState syncState,
      ...
      final Optional<PendingTransactionsMessageSender>
          pendingTransactionsMessageSender,
      final boolean eth65Enabled, final Optional<EIP1559> eip1559) {
    checkState(eth65Enabled == pendingTransactionTracker.isPresent(),
    ...
    final TransactionPool transactionPool = new TransactionPool(
        ...
        new TransactionSender(transactionTracker, transactionsMessageSender,
                              ethContext),
        eth65Enabled ? Optional.of(new PendingTransactionSender(
                           pendingTransactionTracker.get(),
                           pendingTransactionsMessageSender.get(), ethContext))
                     : Optional.empty(),
        syncState, ethContext, transactionTracker, pendingTransactionTracker,
        minTransactionGasPrice, metricsSystem, eip1559);
    final TransactionsMessageHandler transactionsMessageHandler =
        new TransactionsMessageHandler(
            ethContext.getScheduler(),
            new TransactionsMessageProcessor(
                transactionTracker, transactionPool,
    ...
    ethContext.getEthMessages().subscribe(EthPV62.TRANSACTIONS,
