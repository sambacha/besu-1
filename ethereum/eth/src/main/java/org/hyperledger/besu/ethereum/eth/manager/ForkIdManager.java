...
public class ForkIdManager {
  ...
  public ForkIdManager(final Blockchain blockchain, final List<Long> forks) {
    ...
    final Predicate<ForkId> legacyForkIdChecker =
        createForkIDChecker(blockchain, genesisHash, forks,
                            fs
                            -> fs.stream()
                                   .filter(fork -> fork > 0)
                                   .distinct()
                            ...
                            forkAndHashList);
    // if the fork list contains only zeros then we may be in a consortium/dev
    // network
    if (onlyZerosForkBlocks(forks)) {
      this.forkIDCheckers = singletonList(forkId -> true);
    } else {
      final Predicate<ForkId> newForkIdChecker = createForkIDChecker(
          blockchain, genesisHash, forks,
          fs
          -> fs.stream().distinct().collect(Collectors.toUnmodifiableList()),
          new ArrayList<>());
      this.forkIDCheckers =
          Arrays.asList(newForkIdChecker, legacyForkIdChecker);
    }
  }

  private static Predicate<ForkId>
  createForkIDChecker(final Blockchain blockchain, final Hash genesisHash,
    ...
    return eip2124(blockchain, forkNext, forkIds,
                   highestKnownFork(sanitizedForks));
  }
  ...
  public List<ForkId> getForkAndHashList() { return this.forkAndHashList; }
  ...
  private static Predicate<ForkId> eip2124(final Blockchain blockchain,
                                           final long forkNext,
                                           final List<ForkId> forkAndHashList,
                                           final long highestKnownFork) {
    return forkId -> {
      if (forkId == null) {
        return true; // Another method must be used to validate (i.e. genesis
                     // hash)
      }
      // Run the fork checksum validation rule set:
      //   1. If local and remote FORK_CSUM matches, connect.
      //        The two nodes are in the same fork state currently. They might
      //        know of differing future forks, but that's not relevant until
      //        the fork triggers (might be postponed, nodes might be updated to
      //        match).
      //   2. If the remote FORK_CSUM is a subset of the local past forks and
      //   the
      //      remote FORK_NEXT matches with the locally following fork block
      //      number, connect.
      //        Remote node is currently syncing. It might eventually diverge
      //        from us, but at this current point in time we don't have enough
      //        information.
      //   3. If the remote FORK_CSUM is a superset of the local past forks and
      //   can
      //      be completed with locally known future forks, connect.
      //        Local node is currently syncing. It might eventually diverge
      //        from the remote, but at this current point in time we don't have
      //        enough information.
      ...
      if (isHashKnown(forkId.getHash(), forkAndHashList)) {
        if (blockchain.getChainHeadBlockNumber() < forkNext) {
          ...
          if (isForkKnown(forkId.getNext(), highestKnownFork,
                          forkAndHashList)) {
            return isRemoteAwareOfPresent(forkId.getHash(), forkId.getNext(),
                                          highestKnownFork, forkAndHashList);
          } else {
            return false;
          }
        }
  ...
  private static boolean isHashKnown(final Bytes forkHash,
                                     final List<ForkId> forkAndHashList) {
    return forkAndHashList.stream()
        .map(ForkId::getHash)
        .anyMatch(hash -> hash.equals(forkHash));
  }

  private static boolean isForkKnown(final Long nextFork,
                                     final long highestKnownFork,
                                     final List<ForkId> forkAndHashList) {
    return highestKnownFork < nextFork ||
        forkAndHashList.stream()
            .map(ForkId::getNext)
            .anyMatch(fork -> fork.equals(nextFork));
  }

  private static boolean
  isRemoteAwareOfPresent(final Bytes forkHash, final Long nextFork,
                         final long highestKnownFork,
                         final List<ForkId> forkAndHashList) {
    for (final ForkId j : forkAndHashList) {
      if (forkHash.equals(j.getHash())) {
        if (nextFork.equals(j.getNext())) {
          ...
          return highestKnownFork <=
              nextFork; // Remote aware of an additional future fork
        } else {
          return false;
        }
      }
  ...
  private static long createForkIds(final Hash genesisHash,
                                    final List<Long> forks,
                                    final List<ForkId> forkIds) {
    final CRC32 crc = new CRC32();
    crc.update(genesisHash.toArray());
    final List<Bytes> forkHashes =
        new ArrayList<>(List.of(getCurrentCrcHash(crc)));
  ...
  public static class ForkId {
    ...
    public long getNext() { return next.toLong(); }

    public Bytes getHash() { return hash; }
