...
/**
 * Retrieves a sequence of headers, sending out requests repeatedly until all
 * headers are fulfilled. Validates headers as they are received.
 */
public class DownloadHeaderSequenceTask
  ...
  private DownloadHeaderSequenceTask(final ProtocolSchedule protocolSchedule,
  ...
  public static DownloadHeaderSequenceTask
  endingAtHeader(final ProtocolSchedule protocolSchedule,
                 final ProtocolContext protocolContext,
                 final EthContext ethContext, final BlockHeader referenceHeader,
                 final int segmentLength, final int maxRetries,
                 final ValidationPolicy validationPolicy,
                 final MetricsSystem metricsSystem) {
    return new DownloadHeaderSequenceTask(
        protocolSchedule, protocolContext, ethContext, referenceHeader,
        segmentLength, maxRetries, validationPolicy, metricsSystem);
  }
  ...
  endingAtHeader(final ProtocolSchedule protocolSchedule,
    ...
    return new DownloadHeaderSequenceTask(
        protocolSchedule, protocolContext, ethContext, referenceHeader,
        segmentLength, DEFAULT_RETRIES, validationPolicy, metricsSystem);
  }

  @Override
  protected CompletableFuture<List<BlockHeader>>
  executePeerTask(final Optional<EthPeer> assignedPeer) {
    LOG.debug("Downloading headers from {} to {}.", startingBlockNumber,
    ...
    return task.whenComplete((r, t) -> {
      // We're done if we've filled all requested headers
      if (lastFilledHeaderIndex == 0) {
        LOG.debug("Finished downloading headers from {} to {}.",
                  headers[0].getNumber(),
  ...
  private CompletableFuture<PeerTaskResult<List<BlockHeader>>>
  downloadHeaders(final Optional<EthPeer> assignedPeer) {
    ...
    return executeSubTask(() -> {
      ...
      final AbstractGetHeadersFromPeerTask headersTask =
          GetHeadersFromPeerByHashTask.endingAtHash(
              protocolSchedule, ethContext, referenceHash,
              referenceHeaderForNextRequest.getNumber(), count + 1,
              metricsSystem);
  ...
  private CompletableFuture<List<BlockHeader>>
  processHeaders(final PeerTaskResult<List<BlockHeader>> headersResult) {
    return executeWorkerSubTask(ethContext.getScheduler(), () -> {
      ...
      for (final BlockHeader header : headersResult.getResult()) {
        final int headerIndex = Ints.checkedCast(
        ...
        if (child == null) {
          child = (headerIndex == segmentLength - 1) ? referenceHeader
        ...
        if (!validateHeader(child, header)) {
          ...
          headersResult.getPeer().disconnect(
              DisconnectReason.BREACH_OF_PROTOCOL);
          future.completeExceptionally(new InvalidBlockException(
      ...
      future.complete(
          asList(headers).subList(lastFilledHeaderIndex, previousHeaderIndex));
      return future;
    });
  }

  private boolean validateHeader(final BlockHeader child,
                                 final BlockHeader header) {
    final long finalBlockNumber = startingBlockNumber + segmentLength;
    final boolean blockInRange = header.getNumber() >= startingBlockNumber &&
                                 header.getNumber() < finalBlockNumber;
    if (!blockInRange) {
      return false;
    }
    if (child == null) {
