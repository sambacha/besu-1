...
public class FullImportBlockStep implements Consumer<Block> {
  ...
  public FullImportBlockStep(final ProtocolSchedule protocolSchedule,
  ...
  public void accept(final Block block) {
    ...
    final String shortHash = String.format(
        "%s..%s", blockHash.substring(0, 6),
    ...
    if (!importer.importBlock(protocolContext, block,
                              HeaderValidationMode.SKIP_DETACHED)) {
      throw new InvalidBlockException("Failed to import block", blockNumber,
                                      block.getHash());
    }
    ...
    if (blockNumber % 200 == 0) {
      LOG.info("Import reached block {} ({}), Peers: {}", blockNumber,
               shortHash, peerCount);
    }
  }
}
