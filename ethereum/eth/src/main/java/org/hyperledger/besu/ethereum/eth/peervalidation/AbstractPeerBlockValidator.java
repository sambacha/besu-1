...
abstract class AbstractPeerBlockValidator implements PeerValidator {
  ...
  // Wait for peer's chainhead to advance some distance beyond blockNumber
  // before validating
  private final long chainHeightEstimationBuffer;

  AbstractPeerBlockValidator(final ProtocolSchedule protocolSchedule,
  ...
  public AbstractPeerBlockValidator(final ProtocolSchedule protocolSchedule,
                                    final MetricsSystem metricsSystem,
                                    final long blockNumber) {
    this(protocolSchedule, metricsSystem, blockNumber,
         DEFAULT_CHAIN_HEIGHT_ESTIMATION_BUFFER);
  }

  @Override
  public CompletableFuture<Boolean> validatePeer(final EthContext ethContext,
                                                 final EthPeer ethPeer) {
    final AbstractPeerTask<List<BlockHeader>> getHeaderTask =
        GetHeadersFromPeerByNumberTask
            .forSingleNumber(protocolSchedule, ethContext, blockNumber,
                             metricsSystem)
            .setTimeout(Duration.ofSeconds(20))
            .assignPeer(ethPeer);
    return getHeaderTask.run().handle((res, err) -> {
      if (err != null) {
        // Mark peer as invalid on error
        LOG.debug(
            "Peer {} is invalid because required block block ({}) is unavailable: {}",
            ethPeer, blockNumber, err.toString());
        return false;
      }
  ...
  public boolean canBeValidated(final EthPeer ethPeer) {
    return ethPeer.chainState().getEstimatedHeight() >=
        (blockNumber + chainHeightEstimationBuffer);
  }
  ...
  public Duration nextValidationCheckTimeout(final EthPeer ethPeer) {
    ...
    final long distanceToBlock =
        blockNumber - ethPeer.chainState().getEstimatedHeight();
    if (distanceToBlock < 100_000L) {
      return Duration.ofMinutes(1);
    }
    // If the peer is trailing behind, give it some time to catch up before
    // trying again.
