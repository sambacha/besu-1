...
/**
 * Requests bodies from a peer by header, matches up headers to bodies, and
 * returns blocks.
 */
public class GetBodiesFromPeerTask
    extends AbstractPeerRequestTask<List<Block>> {
  private static final Logger LOG = LogManager.getLogger();

  private final ProtocolSchedule protocolSchedule;
  private final List<BlockHeader> headers;
  private final Map<BodyIdentifier, List<BlockHeader>> bodyToHeaders =
      new HashMap<>();

  private GetBodiesFromPeerTask(final ProtocolSchedule protocolSchedule,
  ...
  public static GetBodiesFromPeerTask
  ...
  @Override
  protected Optional<List<Block>> processResponse(final boolean streamClosed,
    ...
    for (final BlockBody body : bodies) {
      final List<BlockHeader> headers =
          bodyToHeaders.get(new BodyIdentifier(body));
      if (headers == null) {
  ...
  private static class BodyIdentifier {
    ...
    public BodyIdentifier(final Bytes32 transactionsRoot,
    ...
    public BodyIdentifier(final List<Transaction> transactions,
