...
@Command(description = "This command evaluates EVM transactions.",
         abbreviateSynopsis = true, name = "evm",
...
public class EvmToolCommand implements Runnable {
  ...
  @Option(names = {"--chain"}, description = "Name of a well know chain")
  ...
  private final EvmToolCommandOptionsModule daggerOptions =
      new EvmToolCommandOptionsModule();
  ...
  void parse(
    ...
    commandLine.registerConverter(Gas.class,
                                  (arg) -> Gas.of(Long.parseUnsignedLong(arg)));
      ...
      final EvmToolComponent component =
          DaggerEvmToolComponent.builder()
              ...
              .genesisFileModule(
                  network == null
                      ? genesisFile == null
                            ? GenesisFileModule.createGenesisModule(
                                  NetworkName.DEV)
      ...
      do {
        ...
        messageFrameStack.add(
            MessageFrame.builder()
                ...
                .blockHashLookup(
                    new BlockHashLookup(blockHeader, component.getBlockchain()))
        ...
        while (!messageFrameStack.isEmpty()) {
          ...
          mcp.process(messageFrame, new EvmToolOperationTracer(
                                        lastLoop, precompileContractRegistry));
          if (lastLoop) {
            ...
            if (messageFrame.getRevertReason().isPresent()) {
              out.println(
                  new String(messageFrame.getRevertReason().get().toArray(),
          ...
          if (lastLoop && messageFrameStack.isEmpty()) {
            final Transaction tx = new Transaction(
                0, Wei.ZERO, Long.MAX_VALUE, Optional.ofNullable(receiver),
                Wei.ZERO, null, callData, sender, Optional.empty());
            ...
            out.println(
                new JsonObject()
                    ...
                    .put("gasTotal", evmGas.plus(intrinsicGasCost)
                                         .asUInt256()
                                         .toShortHexString()));
          }
        }
  ...
  private JsonObject createEvmTraceOperation(final MessageFrame messageFrame,
    ...
    final String error =
        messageFrame.getExceptionalHaltReasons()
            .stream()
            .findFirst()
            .map(ExceptionalHaltReason::getDescription)
            .orElse(messageFrame.getRevertReason()
                        .map(bytes
                             -> new String(bytes.toArrayUnsafe(),
                                           StandardCharsets.UTF_8))
                        .orElse(""));
    ...
    if (currentOp != null) {
      ...
      results.put("gasCost",
                  currentOp.cost(messageFrame).asUInt256().toShortHexString());
    } else {
      ...
      results.put("address",
                  messageFrame.getContractAddress().toShortHexString());
      results.put("contract", precompileContractRegistry
                                  .get(messageFrame.getContractAddress(),
                                       Account.DEFAULT_VERSION)
                                  .getName());
      results.put("gasCost", callOp.gasAvailableForChildCall(caller)
                                 .minus(messageFrame.getRemainingGas())
                                 .toHexString());
    }
    results.put("gas",
                messageFrame.getRemainingGas().asUInt256().toShortHexString());
    if (!showMemory) {
      results.put(
          "memory",
  ...
  private class EvmToolOperationTracer implements OperationTracer {
    ...
    EvmToolOperationTracer(final boolean lastLoop,
                           final PrecompileContractRegistry
                               precompiledContractRegistries) {
      this.lastLoop = lastLoop;
      this.precompiledContractRegistries = precompiledContractRegistries;
    }

    @Override
    public void traceExecution(final MessageFrame frame,
      ...
      if (showJsonResults && lastLoop) {
        final JsonObject op = EvmToolCommand.this.createEvmTraceOperation(
    ...
    @Override
    public void tracePrecompileCall(final MessageFrame frame,
