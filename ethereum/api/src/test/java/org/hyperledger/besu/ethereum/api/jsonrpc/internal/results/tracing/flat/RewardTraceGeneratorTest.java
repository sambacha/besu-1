...
public class RewardTraceGeneratorTest {
  ...
  private final Address ommerBeneficiary = Address.wrap(
      Bytes.fromHexString("0x095e7baea6a6c7c4c2dfeb977efac326af552d87"));
  private final Address blockBeneficiary = Address.wrap(
      Bytes.fromHexString("0x095e7baea6a6c7c4c2dfeb977efac326af552d88"));
  ...
  public void setUp() {
    final BlockBody blockBody =
        new BlockBody(Collections.emptyList(), List.of(ommerHeader));
    final BlockHeader blockHeader =
        gen.header(0x0A, blockBody, new BlockDataGenerator.BlockOptions());
    block = new Block(blockHeader, blockBody);
    when(protocolSchedule.getByBlockNumber(block.getHeader().getNumber()))
  ...
  @Test
  public void
  assertThatTraceGeneratorReturnValidRewardsForMainnetBlockProcessor() {
    ...
    final Action.Builder actionBlockReward =
        Action.builder()
            ...
            .value(blockProcessor
                       .getCoinbaseReward(blockReward,
                                          block.getHeader().getNumber(), 1)
                       .toShortHexString());
    final Trace blocReward = new RewardTrace.Builder()
    ...
    final Action.Builder actionOmmerReward =
        Action.builder()
            ...
            .value(blockProcessor
                       .getOmmerReward(blockReward,
                                       block.getHeader().getNumber(),
                                       ommerHeader.getNumber())
                       .toShortHexString());
    final Trace ommerReward = new RewardTrace.Builder()
                                  .blockHash(block.getHash().toHexString())
                                  .blockNumber(block.getHeader().getNumber())
  ...
  @Test
  public void
  assertThatTraceGeneratorReturnValidRewardsForClassicBlockProcessor() {
    final ClassicBlockProcessor
        .TransactionReceiptFactory transactionReceiptFactory =
        mock(ClassicBlockProcessor.TransactionReceiptFactory.class);
    final ClassicBlockProcessor blockProcessor = new ClassicBlockProcessor(
        transactionProcessor, transactionReceiptFactory, blockReward,
        BlockHeader::getCoinbase, true);
    when(protocolSpec.getBlockProcessor()).thenReturn(blockProcessor);
