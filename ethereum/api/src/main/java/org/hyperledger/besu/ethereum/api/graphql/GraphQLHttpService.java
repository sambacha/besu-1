...
public class GraphQLHttpService {
  ...
  private static final MediaType MEDIA_TYPE_JUST_JSON =
   ...
   * @param dataFetcherContext DataFetcherContext required by GraphQL to finish
   *     it's job
   */
  public GraphQLHttpService(
      final Vertx vertx, final Path dataDir, final GraphQLConfiguration config,
  ...
  public CompletableFuture<?> start() {
    LOG.info("Starting GraphQL HTTP service on {}:{}", config.getHost(),
             config.getPort());
    ...
    router.route("/").method(GET).method(POST).handler(
        this::handleEmptyRequestAndRedirect);
    router.route(GRAPH_QL_ROUTE)
        .method(GET)
        .method(POST)
    ...
    httpServer.requestHandler(router).listen(res -> {
      if (!res.failed()) {
        ...
        LOG.info("GraphQL HTTP service started and listening on {}:{}",
                 config.getHost(), httpServer.actualPort());
        return;
      }
      httpServer = null;
      ...
      if (cause instanceof SocketException) {
        resultFuture.completeExceptionally(
            new GraphQLServiceException(String.format(
  ...
  // Empty Get/Post requests to / will be redirected to /graphql using 308
  // Permanent Redirect
  private void
  handleEmptyRequestAndRedirect(final RoutingContext routingContext) {
  ...
  private void handleGraphQLRequest(final RoutingContext routingContext) {
    try {
      ...
      case POST:
        ...
        if (contentType != null &&
            MediaType.parse(contentType).is(MEDIA_TYPE_JUST_JSON)) {
          ...
          variables =
              Objects.requireNonNullElse(jsonVariables, Collections.emptyMap());
        } else {
      ...
      vertx.executeBlocking(
          ...
          (res) -> {
            ...
            if (res.failed()) {
              ...
              response.end(
                  serialise(new GraphQLErrorResponse(Collections.singletonMap(
                      "errors",
                      Collections.singletonList(Collections.singletonMap(
                          "message", res.cause().getMessage()))))));
            } else {
              final GraphQLResponse graphQLResponse =
                  (GraphQLResponse)res.result();
  ...
  private GraphQLResponse process(final String requestJson,
                                  final String operationName,
                                  final Map<String, Object> variables) {
  ...
  private void handleGraphQLError(final RoutingContext routingContext,
                                  final Exception ex) {
