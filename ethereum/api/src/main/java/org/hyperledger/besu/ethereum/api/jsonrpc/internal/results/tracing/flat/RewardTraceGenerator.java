...
public class RewardTraceGenerator {
  ...
  /**
   * Generates a stream of reward {@link Trace} from the passed {@link Block}
   * data.
   *
  ...
  generateFromBlock(final ProtocolSchedule protocolSchedule,
                    final Block block) {
    ...
    final Wei coinbaseReward =
        protocolSpec.getBlockProcessor().getCoinbaseReward(
            blockReward, blockHeader.getNumber(), ommers.size());
    ...
    flatTraces.add(0, RewardTrace.builder()
