...
public class TimeoutHandler {
  ...
  handler(final Optional<TimeoutOptions> globalOptions,
  ...
  handler(final Optional<TimeoutOptions> globalOptions,
    ...
    return ctx
        -> processHandler(ctx, globalOptions, timeoutOptionsByMethod,
                          decodeJSON);
  }

  private static void
  processHandler(final RoutingContext ctx,
    ...
    try {
      ...
      if (bodyAsString != null) {
        ...
        if (decodeJSON && !json.isEmpty() && json.charAt(0) == '{') {
          ...
          ctx.put(ContextKey.REQUEST_BODY_AS_JSON_OBJECT.name(),
        ...
        methodTimeoutOptions.or(() -> globalOptions)
            .ifPresent(timeoutOptions -> {
              long tid = ctx.vertx().setTimer(
