...
public class TraceTransaction implements JsonRpcMethod {
  ...
  public TraceTransaction(final Supplier<BlockTracer> blockTracerSupplier,
  ...
  private Object resultByTransactionHash(final Hash transactionHash) {
    return blockchainQueries.transactionByHash(transactionHash)
  ...
  private Object traceBlock(final Block block, final Hash transactionHash) {
    ...
    final TransactionTrace transactionTrace =
        blockTracerSupplier.get()
            ...
            .filter(
                trxTrace
                -> trxTrace.getTransaction().getHash().equals(transactionHash))
            .findFirst()
            .orElseThrow();
    return generateTracesFromTransactionTraceAndBlock(protocolSchedule,
