...
public class PendingStateAdapter extends AdapterBase {
  ...
  public Integer getTransactionCount() { return pendingTransactions.size(); }

  public List<TransactionAdapter> getTransactions() {
    return pendingTransactions.getTransactionInfo()
        .stream()
  ...
  getAccount(final DataFetchingEnvironment dataFetchingEnvironment) {
  ...
  getCall(final DataFetchingEnvironment environment) {
    ...
    final Optional<TransactionSimulatorResult> opt =
        transactionSimulator.processAtHead(param);
    if (opt.isPresent()) {
