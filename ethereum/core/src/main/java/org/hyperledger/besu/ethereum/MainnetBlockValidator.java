...
public class MainnetBlockValidator implements BlockValidator {
  ...
  public MainnetBlockValidator(final BlockHeaderValidator blockHeaderValidator,
  ...
  @Override
  public Optional<BlockProcessingOutputs>
  validateAndProcessBlock(final ProtocolContext context, final Block block,
    ...
    if (!maybeParentHeader.isPresent()) {
      LOG.error(
          ...
          header.getNumber(), header.getHash(), header.getParentHash());
    ...
    if (!maybeWorldState.isPresent()) {
      LOG.debug(
          ...
          header.getNumber(), parentHeader.getStateRoot());
      return Optional.empty();
    }
    ...
    if (!blockBodyValidator.validateBody(context, block, receipts,
                                         worldState.rootHash(),
                                         ommerValidationMode)) {
      return Optional.empty();
    }
  ...
  @Override
  public boolean
  fastBlockValidation(final ProtocolContext context, final Block block,
                      final List<TransactionReceipt> receipts,
                      final HeaderValidationMode headerValidationMode,
                      final HeaderValidationMode ommerValidationMode) {
    final BlockHeader header = block.getHeader();
    if (!blockHeaderValidator.validateHeader(header, context,
                                             headerValidationMode)) {
      return false;
    }
