...
public class PrivateMigrationBlockProcessor {
  ...
  public PrivateMigrationBlockProcessor(final ProtocolSpec protocolSpec) {
    this(protocolSpec.getTransactionProcessor(),
  ...
  processBlock(final Blockchain blockchain, final MutableWorldState worldState,
    ...
    for (final Transaction transaction : transactions) {
      ...
      if (Long.compareUnsigned(transaction.getGasLimit(), remainingGasBudget) >
          0) {
        LOG.warn(
  ...
  private boolean rewardCoinbase(final MutableWorldState worldState,
    ...
    final Wei coinbaseReward =
        blockReward.add(blockReward.multiply(ommers.size()).divide(32));
