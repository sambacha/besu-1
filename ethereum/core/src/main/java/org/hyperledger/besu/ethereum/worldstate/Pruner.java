...
public class Pruner {
  ...
  public Pruner(final MarkSweepPruner pruningStrategy,
    ...
    this(pruningStrategy, blockchain, prunerConfiguration,
         // This is basically the out-of-the-box
         // `Executors.newSingleThreadExecutor` except we want the
         // `corePoolSize` to be 0
    ...
    execute(() -> {
      if (state.compareAndSet(State.IDLE, State.RUNNING)) {
        ...
        blockAddedObserverId =
            blockchain.observeBlockAdded(this::handleNewBlock);
      }
    });
  }
