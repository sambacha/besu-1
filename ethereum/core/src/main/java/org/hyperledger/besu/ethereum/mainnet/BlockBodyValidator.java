...
public interface BlockBodyValidator {
   ...
   * @param worldStateRootHash The rootHash defining the world state after
   *     processing this block and all of its transactions.
   * @param ommerValidationMode The validation mode to use for ommer headers
   * @return {@code true} if valid; otherwise {@code false}
   */
  boolean validateBody(ProtocolContext context, Block block,
   ...
   * @return {@code true} if valid; otherwise {@code false}
   */
  boolean validateBodyLight(ProtocolContext context, Block block,
                            List<TransactionReceipt> receipts,
                            final HeaderValidationMode ommerValidationMode);
}
