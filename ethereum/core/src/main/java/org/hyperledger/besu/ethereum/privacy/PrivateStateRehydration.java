...
public class PrivateStateRehydration {
  ...
  public PrivateStateRehydration(
      ...
      final Blockchain blockchain, final ProtocolSchedule protocolSchedule,
  ...
  public void rehydrate(final List<PrivateTransactionWithMetadata>
    ...
    final Optional<Bytes> maybeGroupId =
        privateTransactionWithMetadataList.get(0)
    ...
    for (int i = 0; i < privateTransactionWithMetadataList.size(); i++) {
      ...
      final Hash blockHash =
          getBlockHashForIndex(i, privateTransactionWithMetadataList);

      // if there are multiple pmts in the list we can increment our index i. At
      // the end of the while loop i will be the index of the last PMT (for this
      // group) that is in this block.
      while (i + 1 < privateTransactionWithMetadataList.size() &&
             blockHash.equals(getBlockHashForIndex(
                 i + 1, privateTransactionWithMetadataList))) {
        i++;
      }

      final Hash lastPmtHash = privateTransactionWithMetadataList.get(i)
      ...
      if (transactionLocationOfLastPmtInBlock.isEmpty()) {
        LOG.debug("Rehydartion failed - missing marker transaction for {}",
                  lastPmtHash);
        return;
      }
      ...
      LOG.debug("Rehydrating block {} ({}/{}), {}", blockHash,
                blockHeader.getNumber(), chainHeadBlockNumber,
                block.getBody()
                    .getTransactions()
                    .stream()
      ...
      final PrivateGroupRehydrationBlockProcessor
          privateGroupRehydrationBlockProcessor =
      ...
      final LinkedHashMap<Hash, PrivateTransaction> enclaveMap =
          new LinkedHashMap<>();
      for (int j = 0; j < privateTransactionWithMetadataList.size(); j++) {
        ...
        enclaveMap.put(transactionWithMetadata.getPrivateTransactionMetadata()
                           .getPrivacyMarkerTransactionHash(),
                       transactionWithMetadata.getPrivateTransaction());
      }

      privateGroupRehydrationBlockProcessor.processBlock(
          blockchain, publicWorldState, privateWorldStateArchive,
          privateStateStorage, privateStateRootResolver, block, enclaveMap,
      ...
      // fix the privacy group header block map for the blocks between the
      // current block and the next block containing a pmt for this privacy
      // group
      if (i + 1 < privateTransactionWithMetadataList.size()) {
        rehydratePrivacyGroupHeadBlockMap(
            privacyGroupId, blockHash, blockchain,
    ...
    final long rehydrationDuration =
        System.currentTimeMillis() - rehydrationStartTimestamp;
    LOG.debug("Rehydration took {} seconds", rehydrationDuration / 1000.0);
  }

  protected void rehydratePrivacyGroupHeadBlockMap(
      final Bytes32 privacyGroupId, final Hash hashOfLastBlockWithPmt,
      final Blockchain currentBlockchain, final long from, final long to) {
    for (long j = from + 1; j < to; j++) {
      ...
      privateStateUpdater.putPrivacyGroupHeadBlockMap(
          theBlockHeader.getHash(),
          new PrivacyGroupHeadBlockMap(thePrivacyGroupHeadBlockMap));
      privateStateUpdater.commit();
    }
  }

  private long getBlockNumberForIndex(final int index,
                                      final List<PrivateTransactionWithMetadata>
                                          privateTransactionWithMetadataList) {
    return blockchain
        .getBlockHeader(
            getBlockHashForIndex(index, privateTransactionWithMetadataList))
        .orElseThrow()
        .getNumber();
  }

  private Hash getBlockHashForIndex(final int index,
                                    final List<PrivateTransactionWithMetadata>
                                        privateTransactionWithMetadataList) {
    return blockchain
        .getTransactionLocation(privateTransactionWithMetadataList.get(index)
                                    .getPrivateTransactionMetadata()
                                    .getPrivacyMarkerTransactionHash())
        .orElseThrow()
        .getBlockHash();
