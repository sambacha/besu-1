...
public class PrivacyBlockProcessor implements BlockProcessor {
  ...
  public PrivacyBlockProcessor(
      ...
      final ProtocolSchedule protocolSchedule, final Enclave enclave,
  ...
  public void
  setPublicWorldStateArchive(final WorldStateArchive publicWorldStateArchive) {
  ...
  public Result processBlock(final Blockchain blockchain,
    ...
    privateStateStorage.updater()
        ...
        .commit();
    return blockProcessor.processBlock(blockchain, worldState, blockHeader,
                                       transactions, ommers);
  }

  void maybeRehydrate(final Blockchain blockchain,
                      final BlockHeader blockHeader,
                      final List<Transaction> transactions) {
    transactions.stream()
        .filter(t
          ...
          try {
            ...
            if (actualList.size() > 0) {
              LOG.debug(
                  ...
                  privacyGroupId.toString(), actualList.size(),
                  privateTransactionWithMetadataList.size());
              final PrivateStateRehydration privateStateRehydration =
                  new PrivateStateRehydration(
                      privateStateStorage, blockchain, protocolSchedule,
                      publicWorldStateArchive, privateWorldStateArchive,
                      privateStateRootResolver);
              privateStateRehydration.rehydrate(actualList);
              privateStateStorage.updater()
                  .putAddDataKey(privacyGroupId, addKey)
                  .commit();
            }
  ...
  createActualList(final BlockHeader blockHeader,
                   final List<PrivateTransactionWithMetadata>
                       privateTransactionWithMetadataList,
                   final Bytes32 privacyGroupId) {
    // if we are the member adding another member we do not have to rehydrate
    // if we have been removed from the group at some point we only need to
    // rehydrate from where we were removed if we are a new member we need to
    // rehydrate the complete state

    List<PrivateTransactionWithMetadata> actualList =
        privateTransactionWithMetadataList;
    ...
    if (maybePrivacyGroupHeadBlockMap.isPresent()) {
      final PrivacyGroupHeadBlockMap privacyGroupHeadBlockMap =
          maybePrivacyGroupHeadBlockMap.get();
      ...
      if (lastBlockWithTx != null) {
        ...
        if (nodeLatestStateRoot.equals(latestStateRootFromRehydrationList)) {
          // we are already on the latest state root, which means that we are
          // the member adding a new member
          actualList = Collections.emptyList();
        } else {
          // we are being added, but do not have to rehydrate all private
          // transactions
          ...
          for (int i = 0; i < privateTransactionWithMetadataList.size(); i++) {
            if (!privateTransactionWithMetadataList.get(i)
                     .getPrivateTransactionMetadata()
                     .getPrivacyMarkerTransactionHash()
            ...
            if (privateTransactionWithMetadataList.size() - 1 == i) {
              actualList =
                  Collections.emptyList(); // nothing needs to be re-hydrated
            } else {
              actualList = privateTransactionWithMetadataList.subList(
  ...
  private List<PrivateTransactionWithMetadata>
  deserializeAddToGroupPayload(final Bytes encodedAddToGroupPayload) {
    final ArrayList<PrivateTransactionWithMetadata> deserializedResponse =
        new ArrayList<>();
    final BytesValueRLPInput bytesValueRLPInput =
        new BytesValueRLPInput(encodedAddToGroupPayload, false);
    final int noOfEntries = bytesValueRLPInput.enterList();
    for (int i = 0; i < noOfEntries; i++) {
      deserializedResponse.add(
          PrivateTransactionWithMetadata.readFrom(bytesValueRLPInput));
    }
