...
public abstract class ClassicDifficultyCalculators {
  ...
  private static final long DELAY =
      (CONTINUE_BLOCK - PAUSE_BLOCK) / EXPONENTIAL_DIFF_PERIOD;
  ...
  adjustForDifficultyDelay(final long periodCount,
                           final BigInteger difficulty) {
    return difficulty.add(
        BIGINT_2.pow(Ints.checkedCast(periodCount - DELAY - 2)));
  }

  private static BigInteger
  adjustForDifficultyPause(final long periodCount,
                           final BigInteger difficulty) {
