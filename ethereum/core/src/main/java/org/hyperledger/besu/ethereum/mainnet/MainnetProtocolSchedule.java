...
public class MainnetProtocolSchedule {
   ...
   *
   * @param config {@link GenesisConfigOptions} containing the config options
   *     for the milestone starting points
   * @param privacyParameters the parameters set for private transactions
   * @param isRevertReasonEnabled whether storing the revert reason is for
   *     failed transactions
   * @return A configured mainnet protocol schedule
   */
  public static ProtocolSchedule
  fromConfig(final GenesisConfigOptions config,
    ...
    return new ProtocolScheduleBuilder(config, DEFAULT_CHAIN_ID,
                                       Function.identity(), privacyParameters,
                                       isRevertReasonEnabled)
