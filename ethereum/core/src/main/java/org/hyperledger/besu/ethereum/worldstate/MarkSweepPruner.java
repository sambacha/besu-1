...
public class MarkSweepPruner {
  ...
  public MarkSweepPruner(final WorldStateStorage worldStateStorage,
                         final MutableBlockchain blockchain,
                         final KeyValueStorage markStorage,
                         final ObservableMetricsSystem metricsSystem) {
    this(worldStateStorage, blockchain, markStorage, metricsSystem,
  ...
  public MarkSweepPruner(final WorldStateStorage worldStateStorage,
    ...
    markedNodesCounter = metricsSystem.createCounter(
        BesuMetricCategory.PRUNER, "marked_nodes_total",
        "Total number of nodes marked as in use");
    markOperationCounter = metricsSystem.createCounter(
        BesuMetricCategory.PRUNER, "mark_operations_total",
        "Total number of mark operations performed");

    sweptNodesCounter = metricsSystem.createCounter(
        BesuMetricCategory.PRUNER, "swept_nodes_total",
        "Total number of unused nodes removed");
    sweepOperationCounter = metricsSystem.createCounter(
    ...
    metricsSystem.createLongGauge(
        BesuMetricCategory.PRUNER, "mark_time_duration",
  ...
  public void prepare() {
    // Optimization for the case where the previous cycle was interrupted (like
    // the node was shut down). If the previous cycle was interrupted, there
    // will be marks in the mark storage from last time, causing the first sweep
    // to be smaller than it needs to be.
    clearMarks();

    nodeAddedListenerId =
        worldStateStorage.addNodeAddedListener(this::markNodes);
  }

  public void mark(final Hash rootHash) {
    markOperationCounter.inc();
    markStopwatch.start();
    createStateTrie(rootHash).visitAll(node -> {
  ...
  public void sweepBefore(final long markedBlockNumber) {
    ...
    // Sweep state roots first, walking backwards until we get to a state root
    // that isn't in the storage
    long prunedNodeCount = 0;
    WorldStateStorage.Updater updater = worldStateStorage.updater();
    for (long blockNumber = markedBlockNumber - 1; blockNumber >= 0;
         blockNumber--) {
  ...
  private boolean isMarked(final Bytes32 key) {
    return pendingMarks.contains(key) ||
        markStorage.containsKey(key.toArrayUnsafe());
  }

  private boolean isMarked(final byte[] key) {
    return pendingMarks.contains(Bytes32.wrap(key)) ||
        markStorage.containsKey(key);
  }
  ...
  private void processAccountState(final Bytes value) {
    final StateTrieAccountValue accountValue =
        StateTrieAccountValue.readFrom(RLP.input(value));
  ...
  private void flushPendingMarks() {
    ...
    try {
      final KeyValueStorageTransaction transaction =
          markStorage.startTransaction();
      pendingMarks.forEach(
          node -> transaction.put(node.toArrayUnsafe(), IN_USE));
      transaction.commit();
      pendingMarks.clear();
    } finally {
      markLock.unlock();
