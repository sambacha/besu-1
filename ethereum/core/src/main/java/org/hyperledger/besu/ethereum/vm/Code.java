...
public class Code {
   ...
   */
  public Code(final Bytes bytes) { this.bytes = bytes; }

  public Code() { this(Bytes.EMPTY); }
  ...
  public int getSize() { return bytes.size(); }
   ...
   */
  public boolean isValidJumpDestination(final EVM evm, final MessageFrame frame,
                                        final UInt256 destination) {
    if (!destination.fitsInt())
      return false;

    final int jumpDestination = destination.intValue();
    if (jumpDestination >= getSize())
      return false;

    if (validJumpDestinations == null) {
      // Calculate valid jump destinations
      validJumpDestinations = new BitSet(getSize());
      evm.forEachOperation(this, frame.getContractAccountVersion(),
   ...
   */
  public boolean isValidJumpSubDestination(final EVM evm,
  ...
  public Bytes getBytes() { return bytes; }
