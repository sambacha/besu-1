...
public class DebugOperationTracer implements OperationTracer {
  ...
  public void traceExecution(final MessageFrame frame,
    ...
    try {
      ...
      final Optional<Map<Address, Wei>> maybeRefunds =
          frame.getRefunds().isEmpty() ? Optional.empty()
                                       : Optional.of(frame.getRefunds());
      lastFrame = new TraceFrame(
          pc, opcode, gasRemaining, currentGasCost, frame.getGasRefund(), depth,
          exceptionalHaltReasons, frame.getRecipientAddress(),
          frame.getApparentValue(), inputData, outputData, stack, memory,
          storage, worldUpdater, frame.getRevertReason(), maybeRefunds,
          Optional.ofNullable(frame.getMessageFrameStack().peek())
              .map(MessageFrame::getCode),
          frame.getCurrentOperation().getStackItemsProduced(),
          stackPostExecution, currentOperation.isVirtualOperation(),
          frame.getMaybeUpdatedMemory(), frame.getMaybeUpdatedStorage());
  ...
  public void tracePrecompileCall(final MessageFrame frame,
    ...
    if (!traceFrames.isEmpty()) {
      traceFrames.get(traceFrames.size() - 1)
          .setPrecompiledGasCost(Optional.of(gasRequirement));
    }
  }

  @Override
  public void
  traceAccountCreationResult(final MessageFrame frame,
                             final Optional<ExceptionalHaltReason> haltReason) {
    haltReason.ifPresent(exceptionalHaltReason -> {
      if (!traceFrames.isEmpty()) {
  ...
  private Optional<Map<UInt256, UInt256>>
  captureStorage(final MessageFrame frame) {
    if (!options.isStorageEnabled()) {
      return Optional.empty();
    }
    try {
      final Map<UInt256, UInt256> storageContents =
          new TreeMap<>(frame.getWorldState()
  ...
  private Optional<Bytes[]> captureMemory(final MessageFrame frame) {
    ...
    final Bytes[] memoryContents =
        new Bytes32[frame.memoryWordSize().intValue()];
    for (int i = 0; i < memoryContents.length; i++) {
      memoryContents[i] =
          frame.readMemory(UInt256.valueOf(i * 32L), UINT256_32);
    }
  ...
  public List<TraceFrame> getTraceFrames() { return traceFrames; }
