...
public class Address extends DelegatingBytes
  ...
  protected Address(final Bytes bytes) { super(bytes); }

  public static Address wrap(final Bytes value) {
    checkArgument(value.size() == SIZE,
                  "An account address must be %s bytes long, got %s", SIZE,
                  value.size());
  ...
  public static Address readFrom(final RLPInput input) {
    ...
    if (bytes.size() != SIZE) {
      throw new RLPException(String.format(
          "Address unexpected size of %s (needs %s)", bytes.size(), SIZE));
    }
   ...
   *     <code>ECDSARECOVER
   *     </code> function from Appendix F (Signing Transactions) of the Ethereum
   * Yellow Paper.
   ...
   *
   * @param str An hexadecimal string (with or without the leading '0x')
   *     representing a valid account address.
   * @return The parsed address: {@code null} if the provided string is {@code
   *     null}.
   * @throws IllegalArgumentException if the string is either not hexadecimal,
   *     or not the valid representation of an address.
   */
  @JsonCreator
   ...
   * Parse an hexadecimal string representing an account address.
   *
   * @param str An hexadecimal string representing a valid account address
   *     (strictly 20 bytes).
   * @return The parsed address.
   * @throws IllegalArgumentException if the provided string is {@code null}.
   * @throws IllegalArgumentException if the string is either not hexadecimal,
   *     or not the valid representation of a 20 byte address.
   */
  public static Address fromHexStringStrict(final String str) {
    ...
    checkArgument(value.size() == SIZE,
                  "An account address must be be %s bytes long, got %s", SIZE,
                  value.size());
    return new Address(value);
  }
   ...
   *
   * <p>This implement equation (86) in Section 7 of the Yellow Paper (rev.
   * a91c29c).
   *
   ...
   */
  public static Address contractAddress(final Address senderAddress,
                                        final long nonce) {
   ...
   * @param senderAddress the address of the transaction sender.
   * @param nonce the nonce of this transaction.
   * @param privacyGroupId hash of participants list ordered from Enclave
   *     response.
   * @return The generated address of the created private contract.
   */
  public static Address privateContractAddress(final Address senderAddress,
                                               final long nonce,
  ...
  fromPlugin(final org.hyperledger.besu.plugin.data.Address logger) {
