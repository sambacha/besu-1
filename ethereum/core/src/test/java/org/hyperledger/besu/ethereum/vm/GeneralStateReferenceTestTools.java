...
public class GeneralStateReferenceTestTools {
  ...
  private static TransactionProcessor transactionProcessor(final String name) {
    return REFERENCE_TEST_PROTOCOL_SCHEDULES.getByName(name)
        .getByBlockNumber(0)
  ...
  static {
    final String eips = System.getProperty(
  ...
  private static final JsonTestParameters<?, ?> params =
      JsonTestParameters
          .create(GeneralStateTestCaseSpec.class,
                  GeneralStateTestCaseEipSpec.class)
          .generator((testName, stateSpec, collector) -> {
            final String prefix = testName + "-";
            for (final Map.Entry<String, List<GeneralStateTestCaseEipSpec>>
                     entry : stateSpec.finalStateSpecs().entrySet()) {
              final String eip = entry.getKey();
              final boolean runTest = EIPS_TO_RUN.contains(eip);
              final List<GeneralStateTestCaseEipSpec> eipSpecs =
                  entry.getValue();
              if (eipSpecs.size() == 1) {
  ...
  generateTestParametersForConfig(final String[] filePath) {
  ...
  public static void executeTest(final GeneralStateTestCaseEipSpec spec) {
    ...
    final MutableWorldState worldState =
        new DefaultMutableWorldState(initialWorldState);
    // Several of the GeneralStateTests check if the transaction could
    // potentially consume more gas than is left for the block it's attempted to
    // be included in. This check is performed within the `BlockImporter` rather
    // than inside the `TransactionProcessor`, so these tests are skipped.
    if (transaction.getGasLimit() >
        blockHeader.getGasLimit() - blockHeader.getGasUsed()) {
      return;
    }
    ...
    final TransactionProcessor.Result result = processor.processTransaction(
        ...
        blockHeader.getCoinbase(), new BlockHashLookup(blockHeader, blockchain),
        false, TransactionValidationParams.processingBlock());
    final Account coinbase =
        worldStateUpdater.getOrCreate(spec.blockHeader().getCoinbase());
    if (coinbase != null && coinbase.isEmpty() &&
        shouldClearEmptyAccounts(spec.eip())) {
    ...
    assertThat(worldState.rootHash())
        .withFailMessage("Unexpected world state root hash; computed state: %s",
                         worldState)
