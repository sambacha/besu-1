...
public class ConstantinopleSStoreOperationGasCostTest {
  ...
  @Parameter(value = 1) public int originalValue;

  @Parameter(value = 2) public int expectedGasUsed;

  @Parameter(value = 3) public int expectedGasRefund;
  ...
  public void shouldCalculateGasAccordingToEip1283() {
    ...
    final MessageFrame frame = codeExecutor.executeCode(
        code, Account.DEFAULT_VERSION, gasLimit,
        account
        -> account.setStorageValue(UInt256.ZERO,
                                   UInt256.valueOf(originalValue)));
    assertThat(frame.getState()).isEqualTo(State.COMPLETED_SUCCESS);
    assertThat(frame.getRemainingGas())
        .isEqualTo(Gas.of(gasLimit - expectedGasUsed));
