...
public class BlockchainReferenceTestTools {
  ...
  static {
    final String networks = System.getProperty(
  ...
  private static final JsonTestParameters<?, ?> params =
      JsonTestParameters.create(BlockchainReferenceTestCaseSpec.class)
          .generator((testName, spec, collector) -> {
  ...
  static {
    ...
    params.blacklist(
        "sha3_memSizeQuadraticCost[0-9][0-9]_(|zeroSize|2)_?Istanbul");
  }
  ...
  public static void executeTest(final BlockchainReferenceTestCaseSpec spec) {
    final MutableWorldState worldState =
        spec.getWorldStateArchive()
            .getMutable(spec.getGenesisBlockHeader().getStateRoot())
            .get();
    final BlockHeader genesisBlockHeader = spec.getGenesisBlockHeader();
    assertThat(worldState.rootHash())
        .isEqualTo(genesisBlockHeader.getStateRoot());
    ...
    for (final BlockchainReferenceTestCaseSpec.CandidateBlock candidateBlock :
      ...
      try {
        ...
        final boolean imported = blockImporter.importBlock(
            context, block, validationMode, validationMode);
    ...
    Assertions.assertThat(blockchain.getChainHeadHash())
