...
public abstract class AbstractBlockCreator implements AsyncBlockCreator {
  ...
  public interface ExtraDataCalculator { Bytes get(final BlockHeader parent); }
  ...
  public AbstractBlockCreator(final Address coinbase,
    ...
    this.protocolSpec =
        protocolSchedule.getByBlockNumber(parentHeader.getNumber() + 1);
    blockHeaderFunctions =
        ScheduleBasedBlockHeaderFunctions.create(protocolSchedule);
  }

  /**
   * Create block will create a new block at the head of the blockchain
   * specified in the protocolContext.
   *
   * <p>It will select transactions from the PendingTransaction list for
   * inclusion in the Block body, and will supply an empty Ommers list.
   *
   * <p>Once transactions have been selected and applied to a
   * disposable/temporary world state, the block reward is paid to the relevant
   * coinbase, and a sealable header is constucted.
   *
   * <p>The sealableHeader is then provided to child instances for sealing (i.e.
   * proof of work or otherwise).
   *
  ...
  @Override
  public Block createBlock(final List<Transaction> transactions,
                           final List<BlockHeader> ommers,
                           final long timestamp) {
    return createBlock(Optional.of(transactions), Optional.of(ommers),
                       timestamp);
  }

  private Block createBlock(final Optional<List<Transaction>> maybeTransactions,
                            final Optional<List<BlockHeader>> maybeOmmers,
                            final long timestamp) {
    try {
      final ProcessableBlockHeader processableBlockHeader =
          createPendingBlockHeader(timestamp);
      ...
      final BlockTransactionSelector
          .TransactionSelectionResults transactionResults = selectTransactions(
          processableBlockHeader, disposableWorldState, maybeTransactions);
      ...
      if (!rewardBeneficiary(disposableWorldState, processableBlockHeader,
                             ommers, protocolSpec.getBlockReward(),
      ...
      final SealableBlockHeader sealableBlockHeader =
          BlockHeaderBuilder.create()
              ...
              .transactionsRoot(BodyValidation.transactionsRoot(
                  transactionResults.getTransactions()))
              .receiptsRoot(
                  BodyValidation.receiptsRoot(transactionResults.getReceipts()))
              .logsBloom(
                  BodyValidation.logsBloom(transactionResults.getReceipts()))
      ...
      final BlockHeader blockHeader =
          createFinalBlockHeader(sealableBlockHeader);
      ...
      // TODO(tmm): How are we going to know this has exploded, and thus restart
      // it?
      LOG.trace(
  ...
  private BlockTransactionSelector.TransactionSelectionResults
  selectTransactions(final ProcessableBlockHeader processableBlockHeader,
                     final MutableWorldState disposableWorldState,
    ...
    final BlockTransactionSelector selector = new BlockTransactionSelector(
        transactionProcessor, protocolContext.getBlockchain(),
        disposableWorldState, pendingTransactions, processableBlockHeader,
        transactionReceiptFactory, minTransactionGasPrice,
        minBlockOccupancyRatio, isCancelled::get, miningBeneficiary,
  ...
  private MutableWorldState duplicateWorldStateAtParent() {
    ...
    final MutableWorldState worldState =
        protocolContext.getWorldStateArchive()
            ...
            .orElseThrow(() -> {
              LOG.info(
                  "Unable to create block because world state is not available");
              return new IllegalStateException(
                  "World state not available for block " +
                  parentHeader.getNumber() + " with state root " +
                  parentStateRoot);
            });
  ...
  private List<BlockHeader> selectOmmers() { return Lists.newArrayList(); }

  private ProcessableBlockHeader
  createPendingBlockHeader(final long timestamp) {
    final long newBlockNumber = parentHeader.getNumber() + 1;
    final long gasLimit;
    if (ExperimentalEIPs.eip1559Enabled && protocolSpec.isEip1559()) {
      gasLimit = protocolSpec.getEip1559().orElseThrow().eip1559GasPool(
          newBlockNumber);
    } else {
      gasLimit = gasLimitCalculator.apply(parentHeader.getGasLimit());
    }
    final DifficultyCalculator difficultyCalculator =
        protocolSpec.getDifficultyCalculator();
    final BigInteger difficulty = difficultyCalculator.nextDifficulty(
        timestamp, parentHeader, protocolContext);
    ...
    if (ExperimentalEIPs.eip1559Enabled && protocolSpec.isEip1559()) {
      ...
      if (eip1559.isForkBlock(newBlockNumber)) {
        ...
        baseFee = eip1559.computeBaseFee(
  ...
  boolean rewardBeneficiary(final MutableWorldState worldState,
    ...
    final Wei coinbaseReward =
        protocolSpec.getBlockProcessor().getCoinbaseReward(
            blockReward, header.getNumber(), ommers.size());
    final WorldUpdater updater = worldState.updater();
    final DefaultEvmAccount beneficiary =
        updater.getOrCreate(miningBeneficiary);
    ...
    for (final BlockHeader ommerHeader : ommers) {
      ...
      final DefaultEvmAccount ommerCoinbase =
          updater.getOrCreate(ommerHeader.getCoinbase());
      final Wei ommerReward = protocolSpec.getBlockProcessor().getOmmerReward(
          blockReward, header.getNumber(), ommerHeader.getNumber());
  ...
  protected abstract BlockHeader
