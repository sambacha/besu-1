...
/**
 * Responsible for creating a block, and importing it to the blockchain. This is
 * specifically a mainnet capability (as IBFT would then use the block as part
 * of a proposal round).
 *
 * <p>While the capability is largely functional, it has been wrapped in an
 * object to allow it to be cancelled safely.
 *
 * <p>This class is responsible for mining a single block only - the
 * AbstractBlockCreator maintains state so must be destroyed between block
 * mining activities.
 */
public class BlockMiner<M extends AbstractBlockCreator> implements Runnable {
  ...
  public BlockMiner(final Function<BlockHeader, M> blockCreatorFactory,
  ...
  /**
   * Create a block with the given transactions and ommers. The list of
   * transactions are validated as they are processed, and are not guaranteed to
   * be included in the final block. If transactions must match exactly, the
   * caller must verify they were all able to be included.
   *
   ...
   */
  public Block createBlock(final BlockHeader parentHeader,
                           final List<Transaction> transactions,
                           final List<BlockHeader> ommers) {
    final BlockCreator blockCreator =
        this.blockCreatorFactory.apply(parentHeader);
    final long timestamp =
        scheduler.getNextTimestamp(parentHeader).getTimestampForHeader();
    return blockCreator.createBlock(transactions, ommers, timestamp);
  }

  protected boolean mineBlock() throws InterruptedException {
    // Ensure the block is allowed to be mined - i.e. the timestamp on the new
    // block is sufficiently ahead of the parent, and still within allowable
    // clock tolerance.
    LOG.trace("Started a mining operation.");

    final long newBlockTimestamp =
        scheduler.waitUntilNextBlockCanBeMined(parentHeader);
  ...
  public void cancel() { minerBlockCreator.cancel(); }

  private void notifyNewBlockListeners(final Block block) {
    observers.forEach(obs -> obs.blockMined(block));
  }

  public BlockHeader getParentHeader() { return parentHeader; }
}
