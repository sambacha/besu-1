...
public class EthHashBlockCreatorTest {
  ...
  private static final long BLOCK_1_TIMESTAMP =
      Long.parseUnsignedLong("55ba4224", 16);

  private static final long BLOCK_1_NONCE =
      Long.parseLong("539bd4979fef1ec4", 16);

  private static final Bytes BLOCK_1_EXTRA_DATA = Bytes.fromHexString(
  ...
  public void createMainnetBlock1() throws IOException {
    final ExecutionContextTestFixture executionContextTestFixture =
        ExecutionContextTestFixture.builder()
            .protocolSchedule(new ProtocolScheduleBuilder(
                                  GenesisConfigFile.DEFAULT.getConfigOptions(),
                                  BigInteger.valueOf(42), Function.identity(),
                                  PrivacyParameters.DEFAULT, false)
                                  .createProtocolSchedule())
            .build();

    final EthHashSolver solver =
        new EthHashSolver(Lists.newArrayList(BLOCK_1_NONCE),
                          new EthHasher.Light(), false, Subscribers.none());
    ...
    final PendingTransactions pendingTransactions = new PendingTransactions(
        ...
        executionContextTestFixture.getProtocolContext()
            .getBlockchain()::getChainHeadHeader,
        Optional.empty(), TransactionPoolConfiguration.DEFAULT_PRICE_BUMP);

    final EthHashBlockCreator blockCreator = new EthHashBlockCreator(
        BLOCK_1_COINBASE,
        parent
        -> BLOCK_1_EXTRA_DATA,
        pendingTransactions, executionContextTestFixture.getProtocolContext(),
    ...
    // A Hashrate should not exist in the block creator prior to creating a
    // block
  ...
  public void createMainnetBlock1_fixedDifficulty1() {
    ...
    // If we weren't setting difficulty to 2^256-1 a difficulty of 1 would have
    // caused a IllegalArgumentException at the previous line, as 2^256 is 33
    // bytes.
  }
  ...
  public void rewardBeneficiary_zeroReward_skipZeroRewardsFalse() {
    ...
    final ProcessableBlockHeader header = BlockHeaderBuilder.create()
    ...
    blockCreator.rewardBeneficiary(mutableWorldState, header,
                                   Collections.emptyList(), Wei.ZERO, false);

    assertThat(mutableWorldState.get(BLOCK_1_COINBASE)).isNotNull();
    assertThat(mutableWorldState.get(BLOCK_1_COINBASE).getBalance())
        .isEqualTo(Wei.ZERO);
  }
  ...
  public void rewardBeneficiary_zeroReward_skipZeroRewardsTrue() {
    ...
    blockCreator.rewardBeneficiary(mutableWorldState, header,
                                   Collections.emptyList(), Wei.ZERO, true);
