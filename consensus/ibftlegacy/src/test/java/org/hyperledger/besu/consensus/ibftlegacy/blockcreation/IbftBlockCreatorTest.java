...
public class IbftBlockCreatorTest {
  ...
  public void headerProducedPassesValidationRules() {
    ...
    // Add the local node as a validator (can't propose a block if node is not a
    // validator).
    final Address localAddr =
        Address.extract(Hash.hash(nodeKeys.getPublicKey().getEncodedBytes()));
    final List<Address> initialValidatorList = Arrays.asList(
        ...
        Address.fromHexString(String.format("%020d", 4)), localAddr);
    ...
    final IbftBlockCreator blockCreator = new IbftBlockCreator(
        ...
        parent
        -> new IbftExtraData(Bytes.wrap(new byte[32]), Lists.newArrayList(),
                             null, initialValidatorList)
               .encode(),
        ...
        protContext, protocolSchedule,
        parentGasLimit
        -> parentGasLimit,
        nodeKeys, Wei.ZERO, 0.8, parentHeader);

    final Block block =
        blockCreator.createBlock(Instant.now().getEpochSecond());
