...
public class IbftExtraDataValidationRuleTest {
  ...
  private BlockHeader
  createProposedBlockHeader(final KeyPair proposerKeyPair,
                            final List<Address> validators) {
    final BlockHeaderTestFixture builder = new BlockHeaderTestFixture();
    builder.number(
        1); // must NOT be block 0, as that should not contain seals at all

    // Construct an extraData block and add to a header
    final IbftExtraData initialIbftExtraData = new IbftExtraData(
        Bytes.wrap(new byte[IbftExtraData.EXTRA_VANITY_LENGTH]), emptyList(),
        null, validators);
    builder.extraData(initialIbftExtraData.encode());
    final BlockHeader header = builder.buildHeader();

    // Hash the header (ignoring committer and proposer seals), and create
    // signature
    final Hash proposerSealHash =
        IbftBlockHashing.calculateDataHashForProposerSeal(header,
                                                          initialIbftExtraData);
    final Signature proposerSignature =
        SECP256K1.sign(proposerSealHash, proposerKeyPair);

    // Construct a new extraData block, containing the constructed proposer
    // signature
    final IbftExtraData proposedData = new IbftExtraData(
        Bytes.wrap(new byte[IbftExtraData.EXTRA_VANITY_LENGTH]), emptyList(),
        proposerSignature, validators);
  ...
  createExtraDataWithCommitSeals(final BlockHeader header,
                                 final Collection<KeyPair> committerKeyPairs) {
    final IbftExtraData extraDataInHeader = IbftExtraData.decode(header);

    final Hash headerHashForCommitters =
        IbftBlockHashing.calculateDataHashForCommittedSeal(header,
                                                           extraDataInHeader);
    ...
    return new IbftExtraData(extraDataInHeader.getVanityData(), commitSeals,
  ...
  public void correctlyConstructedHeaderPassesValidation() {
    ...
    final Address proposerAddress = Address.extract(
        Hash.hash(proposerKeyPair.getPublicKey().getEncodedBytes()));
    ...
    assertThat(extraDataValidationRule.validate(header, null, context))
        .isTrue();
  }
  ...
  public void insufficientCommitSealsFailsValidation() {
    ...
    final BlockHeader header =
        createProposedBlockHeader(proposerKeyPair, validators);
  ...
  @Test
  public void
  mismatchingReportedValidatorsVsLocallyStoredListFailsValidation() {
    final BlockHeaderTestFixture builder = new BlockHeaderTestFixture();
    builder.number(
        1); // must NOT be block 0, as that should not contain seals at all
    final KeyPair proposerKeyPair = KeyPair.generate();
    ...
    // Add another validator to the list reported in the IbftExtraData (note, as
    // the
    final List<Address> extraDataValidators = Lists.newArrayList(
        proposerAddress,
        AddressHelpers.calculateAddressWithRespectTo(proposerAddress, 1));
    BlockHeader header =
        createProposedBlockHeader(proposerKeyPair, extraDataValidators);
  ...
  public void committerNotInValidatorListFailsValidation() {
    ...
    final IbftExtraData commitedExtraData = createExtraDataWithCommitSeals(
        header, singletonList(nonValidatorKeyPair));
    builder.extraData(commitedExtraData.encode());
    header = builder.buildHeader();
  ...
  public void ratioOfCommittersToValidatorsAffectValidation() {
    ...
    // The concern in the above is that when using 6 validators, only 1/2 the
    // validators are required to seal a block. All other combinations appear to
    // be safe they always have >50% validators sealing the block.
  }

  private boolean subExecution(final int validatorCount,
    ...
    for (int i = 0; i < validatorCount - 1;
      ...
      validators.add(Address.extract(
          Hash.hash(committerKeyPair.getPublicKey().getEncodedBytes())));
    }

    Collections.sort(validators);
    BlockHeader header = createProposedBlockHeader(proposerKeyPair, validators);
    final IbftExtraData commitedExtraData = createExtraDataWithCommitSeals(
        header, committerKeys.subList(0, committerCount));
