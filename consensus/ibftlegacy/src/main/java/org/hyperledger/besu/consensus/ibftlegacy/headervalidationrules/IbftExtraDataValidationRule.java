...
/**
 * Ensures the byte content of the extraData field can be deserialised into an
 * appropriate structure, and that the structure created contains data matching
 * expectations from preceding blocks.
 */
public class IbftExtraDataValidationRule
  ...
  public boolean validate(final BlockHeader header, final BlockHeader parent,
    ...
    try {
      ...
      if (!Iterables.elementsEqual(ibftExtraData.getValidators(),
                                   sortedReportedValidators)) {
        LOG.trace(
            "Validators are not sorted in ascending order. Expected {} but got {}.",
            sortedReportedValidators, ibftExtraData.getValidators());
        return false;
      }

      if (!Iterables.elementsEqual(ibftExtraData.getValidators(),
                                   storedValidators)) {
        LOG.trace("Incorrect validators. Expected {} but got {}.",
                  storedValidators, ibftExtraData.getValidators());
        return false;
      }

    } catch (final RLPException ex) {
      LOG.trace(
          "ExtraData field was unable to be deserialised into an IBFT Struct.",
          ex);
      return false;
  ...
  private boolean
  validateCommitters(final Collection<Address> committers,
                     final Collection<Address> storedValidators) {
    ...
    if (committers.size() < minimumSealsRequired) {
      LOG.trace(
          ...
          minimumSealsRequired, committers.size());
      return false;
    }

    if (!storedValidators.containsAll(committers)) {
      LOG.trace(
          "Not all committers are in the locally maintained validator list.");
      return false;
    }
