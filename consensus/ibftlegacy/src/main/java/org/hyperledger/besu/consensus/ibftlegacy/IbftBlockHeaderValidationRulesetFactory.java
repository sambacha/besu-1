...
public class IbftBlockHeaderValidationRulesetFactory {
  ...
  /**
   * Produces a BlockHeaderValidator configured for assessing ibft block headers
   * which are to form part of the BlockChain (i.e. not proposed blocks, which
   * do not contain commit seals)
   *
   * @param secondsBetweenBlocks the minimum number of seconds which must elapse
   *     between blocks.
   * @return BlockHeaderValidator configured for assessing ibft block headers
   */
  public static BlockHeaderValidator.Builder
  ibftBlockHeaderValidator(final long secondsBetweenBlocks) {
    return createValidator(secondsBetweenBlocks, true);
  }

  /**
   * Produces a BlockHeaderValidator configured for assessing IBFT proposed
   * blocks (i.e. blocks which need to be vetted by the validators, and do not
   * contain commit seals).
   *
   * @param secondsBetweenBlocks the minimum number of seconds which must elapse
   *     between blocks.
   * @return BlockHeaderValidator configured for assessing ibft block headers
   */
  public static BlockHeaderValidator.Builder
  ibftProposedBlockValidator(final long secondsBetweenBlocks) {
    return createValidator(secondsBetweenBlocks, false);
  }

  private static BlockHeaderValidator.Builder
  createValidator(final long secondsBetweenBlocks,
