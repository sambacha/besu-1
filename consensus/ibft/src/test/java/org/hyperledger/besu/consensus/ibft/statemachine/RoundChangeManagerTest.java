...
public class RoundChangeManagerTest {
  ...
  private final ConsensusRoundIdentifier ri1 =
      new ConsensusRoundIdentifier(2, 1);
  private final ConsensusRoundIdentifier ri2 =
      new ConsensusRoundIdentifier(2, 2);
  private final ConsensusRoundIdentifier ri3 =
      new ConsensusRoundIdentifier(2, 3);
  ...
  public void setup() {
    ...
    final RoundChangePayloadValidator
        .MessageValidatorForHeightFactory messageValidatorFactory = mock(
        RoundChangePayloadValidator.MessageValidatorForHeightFactory.class);
    ...
    final RoundChangeMessageValidator roundChangeMessageValidator =
        new RoundChangeMessageValidator(
            new RoundChangePayloadValidator(
                messageValidatorFactory, validators,
    ...
    when(
        proposalConsistencyValidator.validateProposalMatchesBlock(any(), any()))
        .thenReturn(true);
  }

  private RoundChange
  makeRoundChangeMessage(final NodeKey key,
  ...
  private RoundChange
  makeRoundChangeMessageWithPreparedCert(final NodeKey key,
                                         final ConsensusRoundIdentifier round,
    ...
    final ConsensusRoundIdentifier proposalRound =
        TestHelpers.createFrom(round, 0, -1);
    final Block block =
        TestHelpers.createProposalBlock(validators, proposalRound);
    // Proposal must come from an earlier round.
    final Proposal proposal =
        messageFactory.createProposal(proposalRound, block, Optional.empty());

    final List<Prepare> preparePayloads =
        prepareProviders.stream()
            .map(k -> {
              final MessageFactory prepareFactory = new MessageFactory(k);
              return prepareFactory.createPrepare(proposalRound,
                                                  block.getHash());
            })
    ...
    return messageFactory.createRoundChange(
        round, Optional.of(preparedRoundArtifacts));
  }

  @Test
  public void rejectsInvalidRoundChangeMessage() {
    final RoundChange roundChangeData =
        makeRoundChangeMessage(nonValidatorKey, ri1);
  ...
  public void acceptsValidRoundChangeMessage() {
    ...
    assertThat(manager.appendRoundChangeMessage(roundChangeData)).isEmpty();
    assertThat(manager.roundChangeCache.get(ri2).receivedMessages.size())
        .isEqualTo(1);
  }
  ...
  public void becomesReadyAtThreshold() {
    final RoundChange roundChangeDataProposer =
        makeRoundChangeMessage(proposerKey, ri2);
    final RoundChange roundChangeDataValidator1 =
        makeRoundChangeMessage(validator1Key, ri2);
    assertThat(manager.appendRoundChangeMessage(roundChangeDataProposer))
        .isEqualTo(Optional.empty());
    assertThat(
        manager.appendRoundChangeMessage(roundChangeDataValidator1).isPresent())
        .isTrue();
  }
  ...
  public void doesntReachReadyWhenSuppliedWithDifferentRounds() {
    ...
    assertThat(manager.roundChangeCache.get(ri2).receivedMessages.size())
        .isEqualTo(1);
    assertThat(manager.roundChangeCache.get(ri3).receivedMessages.size())
        .isEqualTo(1);
  }
  ...
  public void discardsRoundPreviousToThatRequested() {
    ...
    final RoundChange roundChangeDataValidator1 =
        makeRoundChangeMessage(validator1Key, ri2);
    final RoundChange roundChangeDataValidator2 =
  ...
  @Test
  public void
  roundChangeMessagesWithPreparedCertificateMustHaveSufficientPrepareMessages() {
    // Specifically, prepareMessage count is ONE LESS than the calculated quorum
    // size (as the proposal acts as the extra msg). There are 3 validators,
    // therefore, should only need 2 prepare message to be acceptable.

    // These tests are run at ri2, such that validators can be found for past
    // round at ri1.
    RoundChange roundChangeData = makeRoundChangeMessageWithPreparedCert(
        proposerKey, ri2, Collections.emptyList());
    assertThat(manager.appendRoundChangeMessage(roundChangeData)).isEmpty();
    assertThat(manager.roundChangeCache.get(ri2)).isNull();

    roundChangeData = makeRoundChangeMessageWithPreparedCert(
