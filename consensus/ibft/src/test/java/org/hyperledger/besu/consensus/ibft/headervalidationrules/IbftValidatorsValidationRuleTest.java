...
public class IbftValidatorsValidationRuleTest {
  ...
  public void correctlyConstructedHeaderPassesValidation() {
    final List<Address> validators =
        Lists.newArrayList(AddressHelpers.ofValue(1), AddressHelpers.ofValue(2),
    ...
    assertThat(validatorsValidationRule.validate(header, null, context))
        .isTrue();
  }
  ...
  mismatchingReportedValidatorsVsLocallyStoredListFailsValidation() {
    ...
    final List<Address> reportedValidators =
        Lists.newArrayList(AddressHelpers.ofValue(2), AddressHelpers.ofValue(3),
                           AddressHelpers.ofValue(4));

    final ProtocolContext context = new ProtocolContext(
        null, null, setupContextWithValidators(storedValidators));
