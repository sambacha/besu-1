...
public class IbftCoinbaseValidationRuleTest {
  ...
  createProposedBlockHeader(final NodeKey proposerNodeKey,
    ...
    final IbftExtraData ibftExtraData = IbftExtraDataFixture.createExtraData(
        header, Bytes.wrap(new byte[IbftExtraData.EXTRA_VANITY_LENGTH]),
        Optional.of(Vote.authVote(Address.fromHexString("1"))), validators,
        committerNodeKeys);
  ...
  public void proposerInValidatorListPassesValidation() {
    ...
    final Address proposerAddress = Address.extract(
        Hash.hash(proposerNodeKey.getPublicKey().getEncodedBytes()));
    ...
    final IbftCoinbaseValidationRule coinbaseValidationRule =
        new IbftCoinbaseValidationRule();

    BlockHeader header =
        createProposedBlockHeader(proposerNodeKey, validators, committers);
  ...
  public void proposerNotInValidatorListFailsValidation() {
    ...
    final Address otherValidatorNodeAddress = Address.extract(
        Hash.hash(otherValidatorNodeKey.getPublicKey().getEncodedBytes()));

    final List<Address> validators =
        Lists.newArrayList(otherValidatorNodeAddress);
    ...
    assertThat(coinbaseValidationRule.validate(header, null, context))
        .isFalse();
  }
}
