...
public class IbftRoundTest {
  ...
  public void setup() {
    ...
    proposedExtraData = new IbftExtraData(Bytes.wrap(new byte[32]), emptyList(),
                                          empty(), 0, emptyList());
  ...
  public void onConstructionRoundTimerIsStarted() {
    final RoundState roundState =
        new RoundState(roundIdentifier, 3, messageValidator);
    new IbftRound(roundState, blockCreator, protocolContext, blockImporter,
                  subscribers, nodeKey, messageFactory, transmitter,
                  roundTimer);
  ...
  public void onReceptionOfValidProposalSendsAPrepareToNetworkPeers() {
    ...
    final IbftRound round = new IbftRound(
        ...
        nodeKey, messageFactory, transmitter, roundTimer);

    round.handleProposalMessage(messageFactory.createProposal(
        roundIdentifier, proposedBlock, Optional.empty()));
    verify(transmitter, times(1))
        .multicastPrepare(roundIdentifier, proposedBlock.getHash());
  ...
  public void singleValidatorImportBlocksImmediatelyOnProposalCreation() {
    final RoundState roundState =
        new RoundState(roundIdentifier, 1, messageValidator);
    final IbftRound round = new IbftRound(
        roundState, blockCreator, protocolContext, blockImporter, subscribers,
        nodeKey, messageFactory, transmitter, roundTimer);
    round.createAndSendProposalMessage(15);
  ...
  @Test
  public void
  twoValidatorNetworkSendsPrepareOnProposalReceptionThenSendsCommitOnCommitReceive() {
    final RoundState roundState =
        new RoundState(roundIdentifier, 2, messageValidator);
    final IbftRound round = new IbftRound(
        roundState, blockCreator, protocolContext, blockImporter, subscribers,
        nodeKey, messageFactory, transmitter, roundTimer);
    ...
    verify(transmitter, times(1))
        .multicastPrepare(roundIdentifier, proposedBlock.getHash());
    verify(transmitter, times(1))
        .multicastCommit(roundIdentifier, proposedBlock.getHash(),
                         localCommitSeal);
    verify(blockImporter, never()).importBlock(any(), any(), any());

    // Receive Commit Message

    round.handleCommitMessage(messageFactory.createCommit(
        roundIdentifier, proposedBlock.getHash(), remoteCommitSeal));

    // Should import block when both commit seals are available.
    final ArgumentCaptor<Block> capturedBlock =
        ArgumentCaptor.forClass(Block.class);
    verify(blockImporter, times(1))
        .importBlock(any(), capturedBlock.capture(), any());
    ...
    assertThat(importedExtraData.getSeals())
        .containsOnly(remoteCommitSeal, localCommitSeal);
  }

  @Test
  public void
  localNodeProposesToNetworkOfTwoValidatorsImportsOnReceptionOfCommitFromPeer() {
    final RoundState roundState =
        new RoundState(roundIdentifier, 2, messageValidator);
    final IbftRound round = new IbftRound(
        roundState, blockCreator, protocolContext, blockImporter, subscribers,
  ...
  @Test
  public void
  aProposalWithAnewBlockIsSentUponReceptionOfARoundChangeWithNoCertificate() {
    final RoundState roundState =
        new RoundState(roundIdentifier, 2, messageValidator);
    final IbftRound round = new IbftRound(
        roundState, blockCreator, protocolContext, blockImporter, subscribers,
    ...
    verify(transmitter, times(1))
        .multicastProposal(eq(roundIdentifier), any(),
                           eq(Optional.of(roundChangeCertificate)));
  }

  @Test
  public void
  aProposalMessageWithTheSameBlockIsSentUponReceptionOfARoundChangeWithCertificate() {
    final ConsensusRoundIdentifier priorRoundChange =
        new ConsensusRoundIdentifier(1, 0);
    final RoundState roundState =
        new RoundState(roundIdentifier, 2, messageValidator);
    final IbftRound round = new IbftRound(
    ...
    final RoundChangeArtifacts roundChangeArtifacts =
        RoundChangeArtifacts.create(
            Collections.singletonList(messageFactory.createRoundChange(
                roundIdentifier,
    ...
    verify(transmitter, times(1))
        .multicastProposal(
            eq(roundIdentifier), blockCaptor.capture(),
    ...
    assertThat(proposedExtraData.getRound())
        .isEqualTo(roundIdentifier.getRoundNumber());

    // Inject a single Prepare message, and confirm the roundState has gone to
    // Prepared (which indicates the block has entered the roundState (note: all
    // msgs are deemed valid due to mocks)
    round.handlePrepareMessage(
        messageFactory.createPrepare(roundIdentifier, proposedBlock.getHash()));
  ...
  @Test
  public void
  creatingNewBlockFromEmptyPreparedCertificateUpdatesInternalState() {
    final RoundState roundState =
        new RoundState(roundIdentifier, 2, messageValidator);
    final IbftRound round = new IbftRound(
        roundState, blockCreator, protocolContext, blockImporter, subscribers,
    ...
    final RoundChangeArtifacts roundChangeArtifacts =
        RoundChangeArtifacts.create(Collections.singletonList(
            messageFactory.createRoundChange(roundIdentifier, empty())));
  ...
  @Test
  public void
  blockIsImportedOnlyOnceIfQuorumCommitsAreReceivedPriorToProposal() {
  ...
  public void exceptionDuringNodeKeySigningDoesNotEscape() {
    ...
    final MessageFactory throwingMessageFactory =
        new MessageFactory(throwingNodeKey);
    when(throwingNodeKey.sign(any()))
        .thenThrow(new SecurityModuleException("Hsm is Offline"));

    final IbftRound round = new IbftRound(
        roundState, blockCreator, protocolContext, blockImporter, subscribers,
        throwingNodeKey, throwingMessageFactory, transmitter, roundTimer);

    round.handleProposalMessage(messageFactory.createProposal(
        roundIdentifier, proposedBlock, Optional.empty()));

    // Verify that no prepare message was constructed by the IbftRound
    assertThat(roundState.constructPreparedRoundArtifacts()
                   .get()
