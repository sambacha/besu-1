...
public class IbftBlockCreatorTest {
  ...
  @Test
  public void
  createdBlockPassesValidationRulesAndHasAppropriateHashAndMixHash() {
    // Construct a parent block.
    final BlockHeaderTestFixture blockHeaderBuilder =
        new BlockHeaderTestFixture();
    blockHeaderBuilder.gasLimit(
        5000); // required to pass validation rule checks.
    ...
    final ProtocolSchedule protocolSchedule = IbftProtocolSchedule.create(
        GenesisConfigFile
            .fromConfig("{\"config\": {\"spuriousDragonBlock\":0}}")
            .getConfigOptions());
    final ProtocolContext protContext =
        new ProtocolContext(blockchain, createInMemoryWorldStateArchive(),
    ...
    final IbftBlockCreator blockCreator = new IbftBlockCreator(
        initialValidatorList.get(0),
        parent
        -> new IbftExtraData(Bytes.wrap(new byte[32]), Collections.emptyList(),
                             Optional.empty(), 0, initialValidatorList)
               .encode(),
        pendingTransactions, protContext, protocolSchedule,
        parentGasLimit -> parentGasLimit, Wei.ZERO, 0.8, parentHeader);

    final int secondsBetweenBlocks = 1;
    final Block block =
        blockCreator.createBlock(parentHeader.getTimestamp() + 1);

    final BlockHeaderValidator rules =
        IbftBlockHeaderValidationRulesetFactory
            .ibftBlockHeaderValidator(secondsBetweenBlocks)
            .build();

    // NOTE: The header will not contain commit seals, so can only do light
    // validation on header.
    final boolean validationResult =
        rules.validateHeader(block.getHeader(), parentHeader, protContext,
    ...
    assertThat(block.getHash())
        .isEqualTo(IbftBlockHashing.calculateDataHashForCommittedSeal(
            header, extraData));
  }
}
