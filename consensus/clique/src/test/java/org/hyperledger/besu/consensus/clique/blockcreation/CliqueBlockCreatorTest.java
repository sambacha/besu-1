...
public class CliqueBlockCreatorTest {
  ...
  @Before
  public void setup() {
    protocolSchedule = CliqueProtocolSchedule.create(
        GenesisConfigFile.DEFAULT.getConfigOptions(), proposerNodeKey, false);

    final Address otherAddress =
        Util.publicKeyToAddress(otherKeyPair.getPublicKey());
    ...
    final Block emptyBlock =
        new Block(TestHelpers.createCliqueSignedBlockHeader(
  ...
  public void proposerAddressCanBeExtractFromAConstructedBlock() {
    ...
    final Bytes extraData = CliqueExtraData.createWithoutProposerSeal(
        Bytes.wrap(new byte[32]), validatorList);

    final Address coinbase = AddressHelpers.ofValue(1);
    final CliqueBlockCreator blockCreator = new CliqueBlockCreator(
        coinbase,
        parent
        -> extraData,
        ...
        gasLimit
        -> gasLimit,
        proposerNodeKey, Wei.ZERO, 0.8, blockchain.getChainHeadHeader(),
        epochManager);
  ...
  public void insertsValidVoteIntoConstructedBlock() {
    ...
    assertThat(createdBlock.getHeader().getNonce())
        .isEqualTo(CliqueBlockInterface.ADD_NONCE);
  ...
  public void insertsNoVoteWhenAuthInValidators() {
    ...
    final Block createdBlock = blockCreator.createBlock(0L);
    assertThat(createdBlock.getHeader().getNonce())
        .isEqualTo(CliqueBlockInterface.DROP_NONCE);
    assertThat(createdBlock.getHeader().getCoinbase())
        .isEqualTo(Address.fromHexString("0"));
  }
