...
public class CliqueMiningTracker {
  ...
  public CliqueMiningTracker(final Address localAddress,
  ...
  public boolean isProposerAfter(final BlockHeader header) {
    final Address nextProposer = CliqueHelpers.getProposerForBlockAfter(
        header, protocolContext.getConsensusState(CliqueContext.class)
                    .getVoteTallyCache());
    return localAddress.equals(nextProposer);
  }

  public boolean canMakeBlockNextRound(final BlockHeader header) {
    return CliqueHelpers.addressIsAllowedToProduceNextBlock(
        localAddress, protocolContext, header);
  }
