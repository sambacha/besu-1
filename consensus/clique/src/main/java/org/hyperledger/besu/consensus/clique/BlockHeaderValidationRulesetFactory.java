...
public class BlockHeaderValidationRulesetFactory {
  ...
  /**
   * Creates a set of rules which when executed will determine if a given block
   * header is valid with respect to its parent (or chain).
   *
   * <p>Specifically the set of rules provided by this function are to be used
   * for a Clique chain.
   *
   * @param secondsBetweenBlocks the minimum number of seconds which must elapse
   *     between blocks.
   * @param epochManager an object which determines if a given block is an epoch
   *     block.
   * @return the header validator.
   */
  public static BlockHeaderValidator.Builder
  cliqueBlockHeaderValidator(final long secondsBetweenBlocks,
    ...
    return new BlockHeaderValidator.Builder()
        ...
        .addRule(new ConstantFieldValidationRule<>(
            "MixHash", BlockHeader::getMixHash, Hash.ZERO))
        .addRule(new ConstantFieldValidationRule<>(
