...
public class CliqueMiningCoordinator
    extends AbstractMiningCoordinator<CliqueBlockMiner> {

  private final CliqueMiningTracker miningTracker;

  public CliqueMiningCoordinator(final Blockchain blockchain,
  ...
  private boolean
  networkBlockBetterThanCurrentMiner(final BlockHeader newChainHeadHeader) {
    final BlockHeader parentHeader =
        currentRunningMiner.get().getParentHeader();
    final long currentMinerTargetHeight = parentHeader.getNumber() + 1;
    if (currentMinerTargetHeight < newChainHeadHeader.getNumber()) {
      return true;
    }

    final boolean nodeIsMining =
        miningTracker.canMakeBlockNextRound(parentHeader);
