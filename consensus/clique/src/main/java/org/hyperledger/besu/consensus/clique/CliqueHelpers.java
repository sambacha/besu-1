...
public class CliqueHelpers {
  ...
  getProposerForBlockAfter(final BlockHeader parent,
                           final VoteTallyCache voteTallyCache) {
    final CliqueProposerSelector proposerSelector =
        new CliqueProposerSelector(voteTallyCache);
    return proposerSelector.selectProposerForNextBlock(parent);
  }

  public static boolean
  addressIsAllowedToProduceNextBlock(final Address candidate,
    ...
    final int minimumUnsignedPastBlocks =
        minimumBlocksSincePreviousSigning(validatorProvider);
    ...
    while (unsignedBlockCount < minimumUnsignedPastBlocks) {
      ...
      final Address parentSigner =
          CliqueHelpers.getProposerOfBlock(localParent);
      ...
      localParent =
          blockchain.getBlockHeader(localParent.getParentHash())
              .orElseThrow(()
  ...
  private static int
  minimumBlocksSincePreviousSigning(final ValidatorProvider validatorProvider) {
    final int validatorCount = validatorProvider.getValidators().size();
    // The number of contiguous blocks in which a signer may only sign 1 (as
    // taken from clique spec)
