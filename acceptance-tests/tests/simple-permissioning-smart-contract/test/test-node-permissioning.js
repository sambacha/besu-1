...
var node1High =
  "0x9bd359fdc3a2ed5df436c3d8914b1532740128929892092b7fcb320c1b62f375";
var node1Low =
...
contract("Permissioning: Nodes", () => {
  it("Should NOT permit any node when none have been added", async () => {
    ...
    let permitted = await proxy.enodeAllowed(
      node1High,
      node1Low,
      node1Host,
      node1Port
  ...
  it("Should compute key", async () => {
    ...
    assert(
      key3 != key2,
      "computed keys for different nodes should be different"
    );
  });
  ...
  it("Should add a node to the whitelist and then permit that node", async () => {
    ...
    permitted = await proxy.enodeAllowed(
      node2High,
      node2Low,
      node2Host,
      node2Port
  ...
  it("Should allow a connection between 2 added nodes", async () => {
    let permitted = await proxy.connectionAllowed(
      node1High,
      node1Low,
      node1Host,
      node1Port,
    ...
    assert.equal(
      ...
      "expected 2 added nodes to work as source <> destination"
    );
  });
  ...
  it("Should remove a node from the whitelist and then NOT permit that node", async () => {
    ...
    permitted = await proxy.connectionAllowed(
      node1High,
      node1Low,
      node1Host,
      node1Port,
    ...
    assert.equal(
      ...
      "expected source disallowed since it was removed"
    );
  });
});
