...
public class Cluster implements AutoCloseable {
  ...
  public Cluster(final ClusterConfiguration clusterConfiguration,
                 final NetConditions net) {
    this(clusterConfiguration, net, BesuNodeRunner.instance());
  }

  public Cluster(final ClusterConfiguration clusterConfiguration,
                 final NetConditions net, final BesuNodeRunner besuNodeRunner) {
  ...
  private Optional<? extends RunnableNode>
  selectAndStartBootnode(final List<? extends RunnableNode> nodes) {
    final Optional<? extends RunnableNode> bootnode =
        nodes.stream()
  ...
  public void runNodeStart(final RunnableNode node) {
    LOG.info("Starting node {} (id = {}...{})", node.getName(),
             node.getNodeId().substring(0, 4), node.getNodeId().substring(124));
  ...
  public void stop() {
    ...
    for (final RunnableNode node : nodes.values()) {
      if (node instanceof BesuNode) {
        besuNodeRunner.stopNode(
            (BesuNode)node); // besuNodeRunner.stopNode also calls node.stop
      } else {
        node.stop();
      }
    }
  ...
  public void stopNode(final BesuNode node) { besuNodeRunner.stopNode(node); }
