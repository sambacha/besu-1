...
public class TomlConfigFileDefaultProvider implements IDefaultValueProvider {
  ...
  public TomlConfigFileDefaultProvider(final CommandLine commandLine,
  ...
  public String defaultValue(final ArgSpec argSpec) {
    ...
    return argSpec.isOption() ? getConfigurationValue(((OptionSpec)argSpec))
                              : null;
  }

  private String getConfigurationValue(final OptionSpec optionSpec) {
    // NOTE: This temporary fix is necessary to make certain options be treated
    // as a multi-value. This can be done automatically by picocli if the object
    // implements Collection.
    final boolean isArray = getKeyName(optionSpec)
                                .map(keyName -> result.isArray(keyName))
                                .orElse(false);
    final String defaultValue;

    // Convert config values to the right string representation for default
    // string value
  ...
  private String getEntryAsString(final OptionSpec spec) {
    // returns the string value of the config line corresponding to the option
    // in toml file or null if not present in the config
  ...
  private Optional<String> getKeyName(final OptionSpec spec) {
    ...
    return Arrays
        .stream(spec.names())
        // remove leading dashes on option name as we can have "--" or "-"
        // options
  ...
  private String getListEntryAsString(final OptionSpec spec) {
    // returns the string representation of the array value of the config line
    // in CLI format corresponding to the option in toml file or null if not
    // present in the config
    return decodeTomlArray(getKeyName(spec)
                               .map(result::getArray)
    ...
    return tomlArrayElements.stream()
        .map(tomlObject -> {
          if (tomlObject instanceof TomlArray) {
            return "[".concat(decodeTomlArray(((TomlArray)tomlObject).toList()))
                .concat("]");
          } else {
  ...
  private String getBooleanEntryAsString(final OptionSpec spec) {
    // return the string representation of the boolean value corresponding to
    // the option in toml file or null if not present in the config
    return getKeyName(spec)
        .map(result::getBoolean)
        .map(Object::toString)
        .orElse(null);
  }

  private String getIntegerEntryAsString(final OptionSpec spec) {
    // return the string representation of the integer value corresponding to
    // the option in toml file or null if not present in the config
  ...
  private void checkConfigurationValidity() {
    if (result == null || result.isEmpty())
      throw new ParameterException(
          commandLine,
          String.format("Unable to read TOML configuration file %s",
                        configFile));
  }
  ...
  private void loadConfigurationFromFile() {
    ...
    if (result == null) {
      try {
        ...
        if (result.hasErrors()) {
          final String errors = result.errors()
                                    .stream()
          ...
          throw new ParameterException(
              commandLine,
              String.format("Invalid TOML configuration: %s", errors));
        }
  ...
  private void checkUnknownOptions(final TomlParseResult result) {
    ...
    final Set<String> unknownOptionsList =
        result.keySet()
            .stream()
            .filter(
                option -> !commandSpec.optionsMap().containsKey("--" + option))
            .collect(Collectors.toSet());

    if (!unknownOptionsList.isEmpty()) {
      final String options =
          unknownOptionsList.size() > 1 ? "options" : "option";
      final String csvUnknownOptions =
          unknownOptionsList.stream().collect(Collectors.joining(", "));
      throw new ParameterException(
          commandLine,
          String.format("Unknown %s in TOML configuration file: %s", options,
                        csvUnknownOptions));
    }
  }
}
