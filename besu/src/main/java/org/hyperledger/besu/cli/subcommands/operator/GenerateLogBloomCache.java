...
public class GenerateLogBloomCache implements Runnable {
  ...
  @Option(
      ...
      description =
          "The block to stop generating the cache (default is last block of the chain).",
      arity = "1..1")
  ...
  public void run() {
    checkPreconditions();
    final Path cacheDir = parentCommand.parentCommand.dataDir().resolve(
        BesuController.CACHE_PATH);
    ...
    try {
      final long finalBlock =
          Math.min(blockchain.getChainHeadBlockNumber(), endBlock);
  ...
  private void checkPreconditions() {
    ...
    checkState(startBlock % BLOCKS_PER_BLOOM_CACHE == 0,
