...
public class BlocksSubCommandTest extends CommandTestAbstract {
  ...
  private static final String EXPECTED_BLOCK_USAGE =
      "Usage: besu blocks [-hV] [COMMAND]" + System.lineSeparator() +
      "This command provides blocks related actions." + System.lineSeparator() +
  ...
  @Test
  public void
  callingBlockImportSubCommandWithoutPathMustDisplayErrorAndUsage() {
  ...
  public void callingBlockImportSubCommandWithPathMustImportBlocksWithThisPath()
    ...
    parseCommand(BLOCK_SUBCOMMAND_NAME, BLOCK_IMPORT_SUBCOMMAND_NAME, "--from",
    ...
    assertThat(pathArgumentCaptor.getValue())
        .isEqualByComparingTo(fileToImport.toPath());
  ...
  public void blocksImport_rlpFormat() throws Exception {
    final File fileToImport = temp.newFile("blocks.file");
    parseCommand(BLOCK_SUBCOMMAND_NAME, BLOCK_IMPORT_SUBCOMMAND_NAME,
                 "--format", "RLP", "--from", fileToImport.getPath());
  ...
  public void blocksImport_rlpFormatMultiple() throws Exception {
    ...
    parseCommand(BLOCK_SUBCOMMAND_NAME, BLOCK_IMPORT_SUBCOMMAND_NAME,
                 "--format", "RLP", fileToImport.getPath(),
                 file2ToImport.getPath(), file3ToImport.getPath());

    verify(rlpBlockImporter, times(3))
        .importBlockchain(pathArgumentCaptor.capture(), any(), anyBoolean());

    assertThat(pathArgumentCaptor.getAllValues())
        .containsExactlyInAnyOrder(fileToImport.toPath(),
                                   file2ToImport.toPath(),
                                   file3ToImport.toPath());
  ...
  public void blocksImport_jsonFormat() throws Exception {
    ...
    final Writer fileWriter =
        Files.newBufferedWriter(fileToImport.toPath(), UTF_8);
    fileWriter.write(fileContent);
    fileWriter.close();

    parseCommand(BLOCK_SUBCOMMAND_NAME, BLOCK_IMPORT_SUBCOMMAND_NAME,
                 "--format", "JSON", "--from", fileToImport.getPath());

    assertThat(commandOutput.toString()).isEmpty();
    assertThat(commandErrorOutput.toString()).isEmpty();

    verify(jsonBlockImporter, times(1))
        .importChain(stringArgumentCaptor.capture());
  ...
  public void blocksExport_missingFileParam() throws IOException {
    ...
    parseCommand("--data-path=" + folder.getRoot().getAbsolutePath(),
                 BLOCK_SUBCOMMAND_NAME, BLOCK_EXPORT_SUBCOMMAND_NAME);
    final String expectedErrorOutputStart =
        "Missing required option '--to=<FILE>'";
    assertThat(commandOutput.toString()).isEmpty();
    assertThat(commandErrorOutput.toString())
        .startsWith(expectedErrorOutputStart);
  ...
  public void blocksExport_noDbDirectory() throws IOException {
    ...
    parseCommand("--data-path=" + folder.getRoot().getAbsolutePath(),
                 BLOCK_SUBCOMMAND_NAME, BLOCK_EXPORT_SUBCOMMAND_NAME, "--to",
                 outputFile.getPath());
    final String expectedErrorOutputStart =
  ...
  public void blocksExport_noStartOrEnd() throws IOException {
    ...
    verify(rlpBlockExporter, times(1))
        .exportBlocks(outputFile, Optional.empty(), Optional.empty());
  }
  ...
  public void blocksExport_withStartAndNoEnd() throws IOException {
    ...
    parseCommand("--data-path=" + folder.getRoot().getAbsolutePath(),
                 BLOCK_SUBCOMMAND_NAME, BLOCK_EXPORT_SUBCOMMAND_NAME, "--to",
                 outputFile.getPath(), "--start-block=1");
    assertThat(commandOutput.toString()).isEmpty();
    assertThat(commandErrorOutput.toString()).isEmpty();

    verify(rlpBlockExporter, times(1))
        .exportBlocks(outputFile, Optional.of(1L), Optional.empty());
  }
  ...
  public void blocksExport_withEndAndNoStart() throws IOException {
    ...
    parseCommand("--data-path=" + folder.getRoot().getAbsolutePath(),
                 BLOCK_SUBCOMMAND_NAME, BLOCK_EXPORT_SUBCOMMAND_NAME, "--to",
                 outputFile.getPath(), "--end-block=10");
    assertThat(commandOutput.toString()).isEmpty();
    assertThat(commandErrorOutput.toString()).isEmpty();

    verify(rlpBlockExporter, times(1))
        .exportBlocks(outputFile, Optional.empty(), Optional.of(10L));
  }
  ...
  public void blocksExport_withStartAndEnd() throws IOException {
    ...
    parseCommand("--data-path=" + folder.getRoot().getAbsolutePath(),
                 BLOCK_SUBCOMMAND_NAME, BLOCK_EXPORT_SUBCOMMAND_NAME, "--to",
                 outputFile.getPath(), "--start-block=1", "--end-block=10");
    assertThat(commandOutput.toString()).isEmpty();
    assertThat(commandErrorOutput.toString()).isEmpty();

    verify(rlpBlockExporter, times(1))
        .exportBlocks(outputFile, Optional.of(1L), Optional.of(10L));
  }
  ...
  public void blocksExport_withOutOfOrderStartAndEnd() throws IOException {
    ...
    parseCommand("--data-path=" + folder.getRoot().getAbsolutePath(),
                 BLOCK_SUBCOMMAND_NAME, BLOCK_EXPORT_SUBCOMMAND_NAME, "--to",
                 outputFile.getPath(), "--start-block=10", "--end-block=1");
    assertThat(commandErrorOutput.toString())
  ...
  public void blocksExport_withEmptyRange() throws IOException {
    ...
    parseCommand("--data-path=" + folder.getRoot().getAbsolutePath(),
                 BLOCK_SUBCOMMAND_NAME, BLOCK_EXPORT_SUBCOMMAND_NAME, "--to",
                 outputFile.getPath(), "--start-block=10", "--end-block=10");
    assertThat(commandErrorOutput.toString())
        .contains(
            "Parameter --end-block (10) must be greater start block (10)");
    assertThat(commandOutput.toString()).isEmpty();
  ...
  public void blocksExport_withInvalidStart() throws IOException {
    ...
    assertThat(commandErrorOutput.toString())
        .contains(
            "Parameter --start-block (-1) must be greater than or equal to zero");
    assertThat(commandOutput.toString()).isEmpty();
  ...
  public void blocksExport_withInvalidEnd() throws IOException {
    ...
    parseCommand("--data-path=" + folder.getRoot().getAbsolutePath(),
                 BLOCK_SUBCOMMAND_NAME, BLOCK_EXPORT_SUBCOMMAND_NAME, "--to",
                 outputFile.getPath(), "--end-block=-1");
    assertThat(commandErrorOutput.toString())
        .contains(
            "Parameter --end-block (-1) must be greater than or equal to zero");
    assertThat(commandOutput.toString()).isEmpty();
  ...
  public void callingBlockExportSubCommandHelpMustDisplayUsage() {
    ...
    assertThat(commandOutput.toString())
        .startsWith(EXPECTED_BLOCK_EXPORT_USAGE);
    assertThat(commandErrorOutput.toString()).isEmpty();
  }

  private void createDbDirectory(final boolean createDataFiles)
      throws IOException {
    final File dbDir = folder.newFolder(BesuController.DATABASE_PATH);
    if (createDataFiles) {
      final Path dataFilePath =
          Paths.get(dbDir.getAbsolutePath(), "0000001.sst");
