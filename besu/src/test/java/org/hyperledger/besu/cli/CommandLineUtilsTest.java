...
public class CommandLineUtilsTest {
  ...
  private abstract static class AbstractTestCommand implements Runnable {
    ...
    @SuppressWarnings("PrivateStaticFinalLoggers") final Logger logger;
    ...
    @Option(names = {"--option-enabled"}, arity = "1")
    final Boolean optionEnabled = true;

    @Option(names = {"--option2"}) final Integer option2 = 2;

    @Option(names = {"--option3"}) final Integer option3 = 3;

    @Option(names = {"--option4"}) final Integer option4 = 4;
  }

  private static class TestCommandWithDeps extends AbstractTestCommand {

    TestCommandWithDeps(final Logger logger) { super(logger); }

    @Override
    public void run() {
      // Check that mining options are able top work or send an error
      CommandLineUtils.checkOptionDependencies(
          logger, commandLine, "--option-enabled", !optionEnabled,
  ...
  private static class TestCommandWithoutDeps extends AbstractTestCommand {
    ...
    TestCommandWithoutDeps(final Logger logger) { super(logger); }
  ...
  public void optionsAreNotExpected() {
    ...
    testCommand.commandLine.parseWithHandlers(
        new RunLast(), defaultExceptionHandler(), "--option-enabled", "false",
        "--option2", "20", "--option3", "30", "--option4", "40");
    verifyOptionsConstraintLoggerCall(mockLogger, "--option2 and --option3",
                                      "--option-enabled");
  ...
  public void optionIsNotExpected() {
    ...
    testCommand.commandLine.parseWithHandlers(
        new RunLast(), defaultExceptionHandler(), "--option-enabled", "false",
        "--option2", "20", "--option4", "40");
    verifyOptionsConstraintLoggerCall(mockLogger, "--option2",
                                      "--option-enabled");
  ...
  public void optionsAreExpected() {
    final AbstractTestCommand testCommand = new TestCommandWithDeps(mockLogger);
    testCommand.commandLine.parseWithHandlers(
        new RunLast(), defaultExceptionHandler(), "--option2", "20",
        "--option3", "30", "--option4", "40");
  ...
  public void noDependencies() {
    final AbstractTestCommand testCommand =
        new TestCommandWithoutDeps(mockLogger);
    testCommand.commandLine.parseWithHandlers(
        new RunLast(), defaultExceptionHandler(), "--option-enabled", "false",
        "--option2", "20", "--option3", "30", "--option4", "40");
    verifyNoMoreInteractions(mockLogger);
   ...
   */
  private void verifyOptionsConstraintLoggerCall(final Logger logger,
                                                 final String dependentOptions,
                                                 final String mainOption) {

    final ArgumentCaptor<String> stringArgumentCaptor =
    ...
    assertThat(stringArgumentCaptor.getAllValues().get(1))
        .isEqualTo(dependentOptions);
