...
public class PrivacyReorgTest {
  ...
  private static final SECP256K1.KeyPair KEY_PAIR =
      SECP256K1.KeyPair.create(SECP256K1.PrivateKey.create(new BigInteger(
  ...
  private static final Bytes32 PRIVACY_GROUP_BYTES32 = Bytes32.fromHexString(
      "0xf250d523ae9164722b06ca25cfa2a7f3c45df96b09e215236f886c876f715bfa");

  // EventEmitter contract binary
  private static final Bytes MOCK_PAYLOAD = Bytes.fromHexString(
  ...
  public void setUp() throws IOException {
    // Start Enclave
    enclave = OrionTestHarnessFactory.create(
    ...
    privateStateRootResolver = new PrivateStateRootResolver(
  ...
  public void privacyGroupHeadIsTracked() {
    ...
    final DefaultBlockchain blockchain =
        (DefaultBlockchain)protocolContext.getBlockchain();
    final PrivateStateStorage privateStateStorage =
        privacyParameters.getPrivateStateStorage();

    final Transaction privacyMarkerTransaction =
        buildMarkerTransaction(getEnclaveKey(enclave.clientUrl()));
    final Block firstBlock = gen.block(getBlockOptionsWithTransaction(
        blockchain.getGenesisBlock(), privacyMarkerTransaction,
    ...
    assertThat(privateStateStorage.getPrivacyGroupHeadBlockMap(
                   blockchain.getGenesisBlock().getHash()))
        .isEmpty();
    assertThat(
        privateStateStorage.getPrivacyGroupHeadBlockMap(firstBlock.getHash()))
        .isNotEmpty();
    assertThat(
        privateStateStorage.getPrivacyGroupHeadBlockMap(firstBlock.getHash()))
        .contains(expected);
    ...
    final Block secondBlock = gen.block(
        getBlockOptionsNoTransaction(firstBlock, secondBlockStateRoot));
  ...
  public void reorgToChainAtEqualHeight() {
    ...
    assertPrivateStateRoot(privateStateRootResolver, blockchain,
                           EMPTY_ROOT_HASH);
  }
  ...
  public void reorgToShorterChain() {
    ...
    final Block firstBlock = gen.block(getBlockOptionsNoTransaction(
        blockchain.getGenesisBlock(), firstBlockStateRoot));

    final String secondBlockStateRoot =
        "0x35c315ee7d272e5b612d454ee87c948657310ab33208b57122f8d0525e91f35e";
    final Block secondBlock = gen.block(getBlockOptionsWithTransaction(
        firstBlock, buildMarkerTransaction(getEnclaveKey(enclave.clientUrl())),
    ...
    final Difficulty remainingDifficultyToOutpace =
        blockchain.getBlockByNumber(1).get().getHeader().getDifficulty().plus(
            blockchain.getBlockByNumber(2).get().getHeader().getDifficulty());
    ...
    final Block forkBlock =
        gen.block(getBlockOptionsNoTransactionWithDifficulty(
            blockchain.getGenesisBlock(),
            remainingDifficultyToOutpace.plus(10L), forkBlockStateRoot));
  ...
  public void reorgToLongerChain() {
    ...
    final Block secondForkBlock =
        gen.block(getBlockOptionsNoTransactionWithDifficulty(
            forkBlock, forkBlock.getHeader().getDifficulty().plus(10L),
    ...
    final Block thirdForkBlock =
        gen.block(getBlockOptionsWithTransactionAndDifficulty(
            secondForkBlock,
            buildMarkerTransaction(getEnclaveKey(enclave.clientUrl())),
  ...
  private void appendBlock(final BesuController besuController,
                           final DefaultBlockchain blockchain,
  ...
  createKeyValueStorageProvider(final Path dataLocation,
    ...
    return new PrivacyKeyValueStorageProviderBuilder()
        ...
        .withCommonConfiguration(
            new BesuConfigurationImpl(dataLocation, dbLocation))
  ...
  private Bytes getEnclaveKey(final URI enclaveURI) {
    final Enclave enclave =
        new EnclaveFactory(Vertx.vertx()).createVertxEnclave(enclaveURI);
    final SendResponse sendResponse = sendRequest(
        enclave, PRIVATE_TRANSACTION, ENCLAVE_PUBLIC_KEY.toBase64String());
  ...
  private SendResponse sendRequest(final Enclave enclave,
    ...
    if (privateTransaction.getPrivacyGroupId().isPresent()) {
      return enclave.send(
          payload, enclavePublicKey,
          privateTransaction.getPrivacyGroupId().get().toBase64String());
    } else {
      final List<String> privateFor = privateTransaction.getPrivateFor()
                                          .get()
                                          .stream()
  ...
  private void assertPrivateStateRoot(
      ...
      final DefaultBlockchain blockchain, final String expected) {
    assertPrivateStateRoot(privateStateRootResolver, blockchain,
                           Hash.fromHexString(expected));
  }

  private void assertPrivateStateRoot(
      final PrivateStateRootResolver privateStateRootResolver,
      final DefaultBlockchain blockchain, final Hash expected) {
    assertThat(privateStateRootResolver.resolveLastStateRoot(
                   Bytes32.wrap(Bytes.fromBase64String(
                       "8lDVI66RZHIrBsolz6Kn88Rd+WsJ4hUjb4hsh29xW/o=")),
                   blockchain.getChainHeadHash()))
        .isEqualTo(expected);
  }

  private BlockDataGenerator.BlockOptions
  getBlockOptionsNoTransaction(final Block parentBlock,
  ...
  private BlockDataGenerator.BlockOptions
  getBlockOptionsWithTransaction(final Block parentBlock,
                                 final Transaction transaction,
                                 final String stateRoot) {
    return getBlockOptions(new BlockDataGenerator.BlockOptions()
                               .addTransaction(transaction)
                               .setReceiptsRoot(Hash.fromHexString(
                                   BLOCK_WITH_SINGLE_TRANSACTION_RECEIPTS_ROOT))
                               .setGasUsed(23176)
                               .setStateRoot(Hash.fromHexString(stateRoot)),
                           parentBlock);
  }
  ...
  private BlockDataGenerator.BlockOptions
  getBlockOptionsNoTransactionWithDifficulty(final Block parentBlock,
  ...
  private BlockDataGenerator.BlockOptions
  getBlockOptionsWithTransactionAndDifficulty(final Block parentBlock,
                                              final Transaction transaction,
                                              final Difficulty difficulty,
                                              final String stateRoot) {
    return getBlockOptions(new BlockDataGenerator.BlockOptions()
  ...
  private BlockDataGenerator.BlockOptions
  getBlockOptions(final BlockDataGenerator.BlockOptions blockOptions,
                  final Block parentBlock) {
    return blockOptions.setBlockNumber(parentBlock.getHeader().getNumber() + 1)
