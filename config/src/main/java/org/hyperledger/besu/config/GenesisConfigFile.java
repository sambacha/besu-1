...
public class GenesisConfigFile {
  ...
  public static GenesisConfigFile mainnet() {
    try {
      return fromConfig(Resources.toString(
          GenesisConfigFile.class.getResource("/mainnet.json"), UTF_8));
  ...
  public static ObjectNode mainnetJsonNode() {
    try {
      final String jsonString = Resources.toString(
          GenesisConfigFile.class.getResource("/mainnet.json"), UTF_8);
  ...
  public static GenesisConfigFile development() {
    try {
      return fromConfig(Resources.toString(
          GenesisConfigFile.class.getResource("/dev.json"), UTF_8));
    } catch (final IOException e) {
      throw new IllegalStateException(e);
    }
  }
  ...
  public GenesisConfigOptions
  getConfigOptions(final Map<String, String> overrides) {
    final ObjectNode config = JsonUtil.getObjectNode(configRoot, "config")
                                  .orElse(JsonUtil.createEmptyObjectNode());
    return JsonGenesisConfigOptions.fromJsonObjectWithOverrides(config,
                                                                overrides);
  }

  public Stream<GenesisAllocation> streamAllocations() {
    return JsonUtil.getObjectNode(configRoot, "alloc")
        .stream()
        .flatMap(
            allocations
            -> Streams.stream(allocations.fieldNames())
                   .map(key
                        -> new GenesisAllocation(
                            key,
                            JsonUtil.getObjectNode(allocations, key).get())));
  }
  ...
  public String getDifficulty() { return getRequiredString("difficulty"); }
  ...
  public long getTimestamp() {
    return parseLong("timestamp",
                     JsonUtil.getValueAsString(configRoot, "timestamp", "0x0"));
  }

  public List<Long> getForks() {
    return JsonUtil.getObjectNode(configRoot, "config")
        .stream()
        .flatMap(node
  ...
  private String getRequiredString(final String key) {
    if (!configRoot.has(key)) {
      throw new IllegalArgumentException(String.format(
          "Invalid genesis block configuration, missing value for '%s'", key));
    }
  ...
  private long parseLong(final String name, final String value) {
    try {
      ...
      throw new IllegalArgumentException(
          "Invalid genesis block configuration, " + name +
          " must be a number but was '" + value + "'");
    }
  }
}
